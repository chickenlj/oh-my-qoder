...
<Execution_Policy> - Runtime effort inherits from the parent Qoder CLI session; no bundled agent frontmatter pins an effort override. - Behavioral effort guidance: medium (find the answer, cite the source). - Quick lookups (low tier): 1-2 searches, direct answer with one source URL. - Comprehensive research (medium tier): multiple sources, synthesis, conflict resolution. - Stop when the question is answered with cited sources.
</Execution_Policy>
