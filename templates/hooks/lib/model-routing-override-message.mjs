...
This environment uses a non-standard model provider (a proxy or alternative endpoint).

How to pass \`model\` on Task/Agent calls:
- Prefer a tier alias: \`model: "high"\`, \`model: "medium"\`, or \`model: "low"\`. OMQ's pre-tool enforcer resolves these to provider-safe IDs when one of these env vars is set: \`DASHSCOPE_DEFAULT_HIGH_MODEL\`, \`DASHSCOPE_DEFAULT_MEDIUM_MODEL\`, \`DASHSCOPE_DEFAULT_LOW_MODEL\`, or \`OMQ_SUBAGENT_MODEL\`.
...
When the session model has no \`[1m]\` suffix, omitting \`model\` is safe UNLESS a custom sub-agent definition pins a bare model ID (e.g. \`model: qwen-max\` in agent frontmatter). When resolver env vars are configured, the enforcer will deny that call with tier-alias guidance; when they are absent, the call is not denied by the enforcer but will fail at the provider. Either way, custom sub-agents should pin tier aliases (not bare model IDs) in their frontmatter. Shipped OMQ agents already do this and are unaffected.

The AGENTS.md instruction "Pass model on Task calls: low, medium, high" applies here — subject to the resolution prerequisites above.
