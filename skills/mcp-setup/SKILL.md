...
Configure Model Context Protocol (MCP) servers to extend Qoder CLI's capabilities with external tools like web search, file system access, and GitHub integration.

## Overview

MCP servers provide additional tools that Qoder CLI agents can use. This skill helps you configure popular MCP servers using the `claude mcp add` command-line interface.

## Step 1: Choose a Setup Path

Use **AskUserQuestion** with **one question at a time** and **no more than 3 options per question**. Recent Qoder CLI builds reject larger option payloads as invalid tool parameters, so keep the MCP selection flow staged.
...
**Options:**
1. **Recommended starter setup** - Fast path for the most common OMQ MCP additions
...
NEXT STEPS:
1. Restart Qoder CLI for changes to take effect
...
- Run /oh-my-qoder:omq-doctor to diagnose issues
...
If the custom server is meant to provide organization-specific reference material to OMQ workflows, prefer a single tool named `get_company_context` that returns markdown via `{ context: string }`.
...
Then point OMQ at the full tool name in `.qoder/omq.jsonc` or `~/.config/qoder-omq/config.jsonc`:
...
- Restart Qoder CLI after configuration
