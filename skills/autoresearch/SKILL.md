...
- You need durable experiment logs under `.omq/autoresearch/`
- You want a supported path for periodic reruns via Qoder CLI native cron
</Use_When>
...
- You want the deprecated `omq autoresearch` CLI flow — it is no longer authoritative
</Do_Not_Use_When>
...
<Required_Artifacts>
Canonical persistent storage lives under `.omq/autoresearch/<mission-slug>/` and/or `.omq/logs/autoresearch/<run-id>/`.
...
<Cron_Integration>
Qoder CLI native cron is a supported integration point for periodic mission enhancement. In v1, prefer documenting/configuring cron inputs over building a large scheduler UI.
...
<Execution_Policy>
- Do not hand execution back to `omq autoresearch`
