...
1. **RUN QA**: Execute verification based on goal type
   ...
   - `--interactive`: Use qa-tester for interactive CLI/service testing:
     ```
     Agent(subagent_type="oh-my-qoder:qa-tester", model="medium", prompt="TEST:
...
3. **ARCHITECT DIAGNOSIS**: Spawn architect to analyze failure
   ...
   ```
   Agent(subagent_type="oh-my-qoder:architect", model="high", prompt="DIAGNOSE FAILURE:
   Goal: [goal type]
...
4. **FIX ISSUES**: Apply architect's recommendations
   ...
   ```
   Agent(subagent_type="oh-my-qoder:executor", model="medium", prompt="FIX:
...
Track state in `.omq/ultraqa-state.json`:
