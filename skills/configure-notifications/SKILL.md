...
This is an interactive, natural-language configuration skill. Walk the user through setup by asking questions with AskUserQuestion. Write the result to `${CLAUDE_CONFIG_DIR:-~/.qoder}/.omq-config.json`.
...
Check if `${CLAUDE_CONFIG_DIR:-~/.qoder}/omq_config.hook.json` exists. If it does, show the current configuration. If not, explain what it does.
...
Read or create `${CLAUDE_CONFIG_DIR:-~/.qoder}/omq_config.hook.json` and merge the new settings:
...
```bash
LEGACY_CONFIG="${CLAUDE_CONFIG_DIR:-$HOME/.qoder}/omq_config.openclaw.json"
