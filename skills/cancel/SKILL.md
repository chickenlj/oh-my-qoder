...
Teams are detected by checking for config files in `${CLAUDE_CONFIG_DIR:-~/.qoder}/teams/`:

```bash
# Check for active teams
TEAM_CONFIGS=$(find "${CLAUDE_CONFIG_DIR:-$HOME/.qoder}"/teams -name config.json -maxdepth 2 2>/dev/null)
```
...
```
For each team found in ${CLAUDE_CONFIG_DIR:-~/.qoder}/teams/:
...
1. Read `${CLAUDE_CONFIG_DIR:-~/.qoder}/teams/*/config.json` to find active teams
