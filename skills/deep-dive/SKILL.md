...
handoff: .omq/specs/deep-dive-{slug}.md
---
...
Save to `.omq/specs/deep-dive-trace-{slug}.md`:
...
After saving:
- Persist `trace_path` in state: `state_write` with `state.trace_path = ".omq/specs/deep-dive-trace-{slug}.md"`
- Keep any ephemeral trace/interview scratch artifacts under `.omq/state/` or `state_write`; do not write temporary files to the repo root or arbitrary working paths.
...
- Save to `.omq/specs/deep-dive-{slug}.md`
- Persist `spec_path` in state: `state_write` with `state.spec_path = ".omq/specs/deep-dive-{slug}.md"`
...
1. **Ralplan -> Autopilot (Recommended)**
   ...
   - Action: Invoke `Skill("oh-my-qoder:plan")` with `--consensus --direct` flags and the spec file path (`spec_path` from state) as context. The `--direct` flag skips the omq-plan skill's interview phase (the deep-dive interview already gathered requirements), while `--consensus` triggers the Planner/Architect/Critic loop. When consensus completes and produces a plan in `.omq/plans/`, invoke `Skill("oh-my-qoder:autopilot")` with the consensus plan as Phase 0+1 output — autopilot skips both Expansion and Planning, starting directly at Phase 2 (Execution).
   - Pipeline: `deep-dive spec -> omq-plan --consensus --direct -> autopilot execution`
...
- Use `Agent(subagent_type="oh-my-qoder:explore", model="low")` for brownfield codebase exploration (Phase 1)
...
- Use `Write` tool to save trace result to `.omq/specs/deep-dive-trace-{slug}.md` and final spec to `.omq/specs/deep-dive-{slug}.md`; use `.omq/state/` or `state_write` for ephemeral artifacts
...
[Phase 5] Spec ready. User selects ralplan -> autopilot.
  -> omq-plan --consensus --direct runs on the spec
...
- [ ] Phase 3 saves trace result to `.omq/specs/deep-dive-trace-{slug}.md` with per-lane critical unknowns
...
- [ ] Final spec saved to `.omq/specs/deep-dive-{slug}.md` in standard deep-interview format
...
- [ ] Phase 5 "Ralplan -> Autopilot" option explicitly invokes autopilot after omq-plan consensus completes
...
- [ ] `slug`, `trace_path`, `spec_path` persisted in state for resume resilience; ephemeral artifacts stayed under `.omq/state/` or `state_write`
</Final_Checklist>
...
Deep-dive's output (`.omq/specs/deep-dive-{slug}.md`) feeds into the standard omq pipeline:

```
/deep-dive "problem"
  -> Trace (3 parallel lanes) + Interview (Socratic Q&A)
  -> Spec: .omq/specs/deep-dive-{slug}.md

  -> /omq-plan --consensus --direct (spec as input)
    -> Planner/Architect/Critic consensus
    -> Plan: .omq/plans/ralplan-*.md
