...
- User already has an active Qoder CLI `/goal` and only wants that native goal loop monitored -- adopt the existing `/goal` explicitly or use artifact-only Ultragoal notes instead of starting Ralph as a competing persistence loop
  </Do_Not_Use_When>
...
- If a Qoder CLI `/goal` is mentioned, treat it as a native session-loop handoff/evidence source only and use the deterministic conflict policies `refuse`, `adopt_existing`, and `artifact_only` rather than non-deterministic warning handling. Ralph remains the OMQ loop authority for this run; do not claim `/goal` independently ran tests or read files, and do not treat evaluator success as a substitute for Ralph reviewer verification.
  </Execution_Policy>
...
7. **Reviewer verification** (tiered, against acceptance criteria):
   ...
   - **On APPROVAL: immediately proceed to Step 7.5 in the same turn. Do NOT pause to report the verdict to the user — reporting happens only at Step 8 (`/oh-my-qoder:cancel`) or on rejection (Step 9). Treating an approved verdict as a reporting checkpoint is a polite-stop anti-pattern.**
...
- **ai-slop-cleaner is a SKILL, not an agent.** Do NOT call it via `Agent(subagent_type="oh-my-qoder:ai-slop-cleaner")` — that subagent type does not exist and the call will fail with "Agent type not found". If you see that error, retry with the Skill tool — do NOT substitute a similarly-named agent like `code-simplifier` as a "closest match".
...
8. **On approval**: After Step 7.6 passes (with Step 7.5 completed, or skipped via `--no-deslop`), run `/oh-my-qoder:cancel` to cleanly exit and clean up all state files
...
- Use `Agent(subagent_type="oh-my-qoder:architect", ...)` for architect verification cross-checks when changes are security-sensitive, architectural, or involve complex multi-system integration
- Use `Agent(subagent_type="oh-my-qoder:critic", ...)` when `--critic=critic`
...
- **Skill vs agent invocation**: `ai-slop-cleaner` is a skill, invoke via `Skill("ai-slop-cleaner")`. `architect`, `critic`, `executor` etc. are agents, invoke via `Agent(subagent_type="oh-my-qoder:<name>")`. If you ever get "Agent type ... not found" for an `oh-my-qoder:<name>` identifier, the item is a skill — retry with the Skill tool. Do NOT substitute a similarly-named agent as a "closest match".
  </Tool_Usage>
...
Agent(subagent_type="oh-my-qoder:executor", model="low", prompt="Add type export for UserConfig")
Agent(subagent_type="oh-my-qoder:executor", model="medium", prompt="Implement the caching layer for API responses")
Agent(subagent_type="oh-my-qoder:executor", model="high", prompt="Refactor auth module to support OAuth2 flow")
...
- Stop when the user says "stop", "cancel", or "abort" -- run `/oh-my-qoder:cancel`
...
- [ ] `/oh-my-qoder:cancel` run for clean state cleanup
</Final_Checklist>
...
- **Plan id (when applicable):** Two ralph runs in the same workspace will conflict on `prd.json`. Use distinct session IDs (the hook payload session_id is already isolated per Qoder CLI session). For parallel ultragoal-backed ralph runs, use `--plan-id`.
