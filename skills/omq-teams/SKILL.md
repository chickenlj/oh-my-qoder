...
```bash
/omq-teams 2:claude "implement auth module with tests"
...
- Reject unsupported agent types up front. `/omq-teams` only supports **`claude`**, **`codex`**, and **`gemini`**.
- If the user asks for an unsupported type such as `expert`, explain that `/omq-teams` launches external CLI workers only.
- For native Qoder CLI team agents/roles, direct them to **`/oh-my-qoder:team`** instead.
...
- Do not anchor the launch cwd to only the repo containing `.omq/plans/...` when
...
- If no safe shared workspace root can be identified, do not launch `/omq-teams`.
