...
Recommendation matrix for which model to pair with each OMQ/OMO agent, framed
...
agents:
  ...
  Hyperplan:   { model: qwen-max }
  Sisyphus:    { model: qwen-plus }
  Hephaestus:  { model: qwen-plus }   # never GPT-*
  Oracle:      { model: qwen-max }
  Aletheia:    { model: qwen-plus }
  Hermes:      { model: qwen-plus }
```
...
```yaml
agents:
  Prometheus:  { model: qwen-plus }
  Hyperplan:   { model: qwen-plus }
