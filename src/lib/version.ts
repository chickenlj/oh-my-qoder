...
export function getRuntimePackageVersion(): string {
  ...
  // When package.json is missing (e.g. Qoder CLI plugin system didn't copy it),
...
/**
 * Detect whether OMQ is running from a local fork / dev install rather
 ...
 *    payload came from a fork (e.g. Qoder CLI's plugin cache copied
...
export function isRuntimePackageLocal(): boolean {
  try {
    ...
    // a parent dir like ~/.qoder/plugins/marketplaces/omq is the junction).
    let cursor = pkgRoot;
    for (let i = 0; i < 6; i++) {
