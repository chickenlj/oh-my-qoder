 ...
 *
 * Provides a single shared database at .omq/state/jobs.db for both providers.
...
function getDbPath(cwd: string): string {
  return join(getOmqRoot(cwd), "state", "jobs.db");
}
...
function ensureStateDir(cwd: string): void {
  const stateDir = join(getOmqRoot(cwd), "state");
 ...
 *
 * @param promptsDir - Path to the .omq/prompts/ directory
