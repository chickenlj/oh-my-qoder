 ...
 *
 * Single entry point for all OMQ security settings.
 * Two layers of configuration:
 *
 * 1. OMQ_SECURITY env var — master switch
...
export interface SecurityConfig {
  ...
  /** Disable project-level .omq/skills/ loading */
 ...
 * Precedence: config file > OMQ_SECURITY env > defaults
 */
export function getSecurityConfig(): SecurityConfig {
  if (cachedConfig) return cachedConfig;

  const isStrict = process.env.OMQ_SECURITY === "strict";
