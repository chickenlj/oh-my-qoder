...
try {
  const probe = join(tmpdir(), `omq-symlink-probe-${process.pid}`);
  const probeTarget = join(tmpdir(), `omq-symlink-target-${process.pid}`);
...
describe('worktree-paths', () => {
  ...
  describe('resolveOmqPath', () => {
    it('should resolve paths under .omq directory', () => {
      const result = resolveOmqPath('state/ralph.json', TEST_DIR);
      expect(result).toBe(join(TEST_DIR, '.omq', 'state', 'ralph.json'));
    });

    it('should reject paths that escape .omq boundary', () => {
      expect(() => resolveOmqPath('../secret.txt', TEST_DIR)).toThrow('path traversal');
    });
  });
  ...
  describe('resolveStatePath', () => {
    it('should resolve state file paths with -state suffix', () => {
      ...
      expect(result).toBe(join(TEST_DIR, '.omq', 'state', 'ralph-state.json'));
    });

    it('should handle input already having -state suffix', () => {
      const result = resolveStatePath('ultrawork-state', TEST_DIR);
      expect(result).toBe(join(TEST_DIR, '.omq', 'state', 'ultrawork-state.json'));
    });
  ...
  describe('ensureOmqDir', () => {
    it('should create directories under .omq', () => {
      const result = ensureOmqDir('state', TEST_DIR);
      expect(result).toBe(join(TEST_DIR, '.omq', 'state'));
  ...
  describe('helper functions', () => {
    it('getWorktreeNotepadPath returns correct path', () => {
      ...
      expect(result).toBe(join(TEST_DIR, '.omq', 'notepad.md'));
    });

    it('getWorktreeProjectMemoryPath returns correct path', () => {
      const result = getWorktreeProjectMemoryPath(TEST_DIR);
      expect(result).toBe(join(TEST_DIR, '.omq', 'project-memory.json'));
    });

    it('getOmqRoot returns correct path', () => {
  ...
  describe('ensureAllOmqDirs', () => {
    it('should create all standard .omq subdirectories', () => {
      ensureAllOmqDirs(TEST_DIR);

      expect(existsSync(join(TEST_DIR, '.omq'))).toBe(true);
      expect(existsSync(join(TEST_DIR, '.omq', 'state'))).toBe(true);
      expect(existsSync(join(TEST_DIR, '.omq', 'plans'))).toBe(true);
      expect(existsSync(join(TEST_DIR, '.omq', 'research'))).toBe(true);
      expect(existsSync(join(TEST_DIR, '.omq', 'logs'))).toBe(true);
      expect(existsSync(join(TEST_DIR, '.omq', 'notepads'))).toBe(true);
      expect(existsSync(join(TEST_DIR, '.omq', 'drafts'))).toBe(true);
    });
  });
  ...
  // ==========================================================================
  // OMQ_STATE_DIR TESTS (Issue #1014)
  // ==========================================================================
  ...
  describe('getOmqRoot with OMQ_STATE_DIR (Issue #1014)', () => {
    it('should return default .omq path when OMQ_STATE_DIR is not set', () => {
      delete process.env.OMQ_STATE_DIR;
      const result = getOmqRoot(TEST_DIR);
      expect(result).toBe(join(TEST_DIR, '.omq'));
    });

    it('should return centralized path when OMQ_STATE_DIR is set', () => {
      const stateDir = mkdtempSync(join(tmpdir(), 'omq-state-dir-'));
      try {
  ...
  describe('path functions with OMQ_STATE_DIR', () => {
    let stateDir: string;

    beforeEach(() => {
      stateDir = mkdtempSync(join(tmpdir(), 'omq-state-dir-paths-'));
      process.env.OMQ_STATE_DIR = stateDir;
    });
    ...
    it('ensureAllOmqDirs should create dirs under centralized path', () => {
      ...
      // Legacy .omq/ should NOT be created
      expect(existsSync(join(TEST_DIR, '.omq'))).toBe(false);
    });
  ...
  describe('workspace marker (.omq-workspace)', () => {
    // Use resolve() so expectations match getOmqRoot's internally-resolved path
    ...
    it('getOmqRoot anchors to marker dir when marker exists in cwd', () => {
      ...
      fs.writeFileSync(join(workspaceDir, '.omq-workspace'), '');
      clearWorktreeCache();
      const result = getOmqRoot(workspaceDir);
      expect(result).toBe(join(workspaceDir, '.omq'));
    });

    it('getOmqRoot walks up from subdir to find marker', () => {
      const fs = require('node:fs');
      fs.writeFileSync(join(workspaceDir, '.omq-workspace'), '');
      clearWorktreeCache();
      const result = getOmqRoot(subrepoDir);
      expect(result).toBe(join(workspaceDir, '.omq'));
    });

    it('getOmqRoot prefers marker over a sub-git-repo root', () => {
      const fs = require('node:fs');
      fs.writeFileSync(join(workspaceDir, '.omq-workspace'), '');
      try {
    ...
    it('getProjectIdentifier honors explicit id from marker', () => {
      const fs = require('node:fs');
      fs.writeFileSync(
        join(workspaceDir, '.omq-workspace'),
  ...
  describe('workspace marker edge cases', () => {
    ...
    beforeEach(() => {
      clearWorktreeCache();
      rootA = resolve(mkdtempSync(join(tmpdir(), 'omq-ws-edge-A-')));
    });
    ...
    it('nested markers: inner workspace wins over outer', () => {
      // Structure: rootA/.omq-workspace  AND  rootA/B/.omq-workspace
      ...
      writeFileSync(join(rootA, '.omq-workspace'), '');
      writeFileSync(join(innerB, '.omq-workspace'), '');
    ...
    it('OMQ_STATE_DIR overrides workspace marker: getOmqRoot returns centralized path', () => {
      const stateDir = mkdtempSync(join(tmpdir(), 'omq-state-override-'));
      try {
        // Drop a workspace marker — without OMQ_STATE_DIR it would steer to rootA/.omq
        writeFileSync(join(rootA, '.omq-workspace'), '');
        clearWorktreeCache();

        process.env.OMQ_STATE_DIR = stateDir;
        const result = getOmqRoot(rootA);

        // Must use centralized path, not the workspace-anchored .omq
        expect(result).not.toBe(join(rootA, '.omq'));
    ...
    it('invalid JSON in marker: readWorkspaceMarkerConfig returns {} gracefully', () => {
      writeFileSync(join(rootA, '.omq-workspace'), '{ not valid json !!!');
    ...
    it.skipIf(process.platform === 'win32' && !canSymlink)(
      ...
      () => {
        const realDir = mkdtempSync(join(tmpdir(), 'omq-ws-real-'));
        const linkDir = join(tmpdir(), `omq-ws-link-${process.pid}`);
        try {
          writeFileSync(join(realDir, '.omq-workspace'), '');
  ...
  describe('warnSiblingRetrofit + clearSiblingRetrofitWarnings', () => {
    ...
    beforeEach(() => {
      anchorDir = resolve(mkdtempSync(join(tmpdir(), 'omq-sibling-anchor-')));
    ...
    it('warns once when siblings have pre-existing .omq/state dirs', () => {
      const siblingA = join(anchorDir, 'repoA');
      const siblingB = join(anchorDir, 'repoB');
      mkdirSync(join(siblingA, '.omq', 'state'), { recursive: true });
      mkdirSync(join(siblingB, '.omq', 'state'), { recursive: true });
      ...
      expect(written).toContain(join(siblingA, '.omq'));
      expect(written).toContain(join(siblingB, '.omq'));
    });

    it('does not warn when no sibling has .omq/state', () => {
      const siblingA = join(anchorDir, 'repoA');
    ...
    it('disk marker is written under {anchor}/.omq/state/sibling-retrofit-warned-{sid}.json', () => {
      const siblingA = join(anchorDir, 'repoA');
      mkdirSync(join(siblingA, '.omq', 'state'), { recursive: true });
      ...
      const markerPath = join(anchorDir, '.omq', 'state', `sibling-retrofit-warned-${sessionId}.json`);
    ...
    it('clearSiblingRetrofitWarnings removes disk markers and allows re-warn', () => {
      ...
      clearSiblingRetrofitWarnings(join(anchorDir, '.omq'));
  ...
  describe('findWorkspaceRoot home-boundary', () => {
    ...
    beforeEach(() => {
      ...
      fakeHome = resolve(mkdtempSync(join(tmpdir(), 'omq-fakehome-')));
    ...
    it('marker EXACTLY at home dir is NOT returned (null)', () => {
      writeFileSync(join(fakeHome, '.omq-workspace'), '');
      clearWorktreeCache();
    ...
    it('marker BELOW home IS found', () => {
      ...
      writeFileSync(join(projectDir, '.omq-workspace'), '');
      clearWorktreeCache();
