 ...
 *
 * Storage: .omq/state/shared-memory/{namespace}/{key}.json
 *
...
import { getOmqRoot } from './worktree-paths.js';
...
export function isSharedMemoryEnabled(): boolean {
  try {
    const configPath = join(getQoderConfigDir(), CONFIG_FILE_NAME);
...
function getNamespaceDir(namespace: string, worktreeRoot?: string): string {
  ...
  return join(omqRoot, SHARED_MEMORY_DIR, namespace);
}
...
export function cleanupExpired(
  ...
  const omqRoot = getOmqRoot(worktreeRoot);
  const sharedMemDir = join(omqRoot, SHARED_MEMORY_DIR);
