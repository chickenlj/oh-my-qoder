 ...
 *
 * Single source of truth for the `omq-hud.mjs` statusline wrapper body
 * used by `src/installer/index.ts` (Path A: `omq setup` / npm).
 *
 * The JS mirror lives at `scripts/lib/hud-wrapper-template.mjs` and is
 * used by `scripts/plugin-setup.mjs` (Path B: Qoder CLI plugin
