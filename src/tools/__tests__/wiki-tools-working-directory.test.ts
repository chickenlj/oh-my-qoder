...
describe('wiki tools workingDirectory', () => {
  ...
  let originalOmqStateDir: string | undefined;

  beforeEach(() => {
    originalCwd = process.cwd();
    originalOmqStateDir = process.env.OMQ_STATE_DIR;
  ...
  afterEach(() => {
    ...
    clearWorktreeCache();
    if (originalOmqStateDir === undefined) {
      delete process.env.OMQ_STATE_DIR;
    } else {
      process.env.OMQ_STATE_DIR = originalOmqStateDir;
    }
  ...
  it('writes and reads wiki pages in the provided linked worktree instead of process cwd', async () => {
    ...
    expect(existsSync(join(linked, '.omq', 'wiki', 'linked-worktree-page.md'))).toBe(true);
    expect(existsSync(join(primary, '.omq', 'wiki'))).toBe(false);
