...
describe('memory-tools payload validation', () => {
  ...
  it('should write to centralized project memory without creating a local file when OMQ_STATE_DIR is set', async () => {
    const stateDir = '/tmp/memory-tools-centralized-state';
    rmSync(stateDir, { recursive: true, force: true });
    mkdirSync(stateDir, { recursive: true });
    rmSync(join(TEST_DIR, '.omq'), { recursive: true, force: true });
    ...
    try {
      ...
      expect(existsSync(join(TEST_DIR, '.omq', 'project-memory.json'))).toBe(false);
      expect(result.isError).toBeUndefined();
    } finally {
      rmSync(stateDir, { recursive: true, force: true });
    }
  ...
  it('should add a directive when existing memory lacks userDirectives', async () => {
    const memoryPath = join(TEST_DIR, '.omq', 'project-memory.json');
