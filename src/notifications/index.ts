 ...
 *
 * Multi-platform lifecycle notifications for oh-my-qoder.
...
export async function notify(
  ...
  // OMQ_NOTIFY=0 suppresses all CCNotifier events (set by `omq --notify false`)
  if (process.env.OMQ_NOTIFY === '0') {
    return null;
  }
    ...
    if (
      ...
      try {
        ...
        const rawTail = payload.projectPath
          ? getNewPaneTail(payload.tmuxPaneId, join(getOmqRoot(payload.projectPath), "state"), tailLines)
