...
describe("getNotificationConfig - file + env deep merge", () => {
  ...
  it("returns env-only config when no file exists", () => {
    vi.stubEnv("OMQ_DISCORD_NOTIFIER_BOT_TOKEN", "env-token");
  ...
  it("merges env discord-bot into file config that lacks it", () => {
    ...
    vi.stubEnv("OMQ_DISCORD_NOTIFIER_CHANNEL", "env-channel-id");
  ...
  it("merges env telegram into file config that only has discord", () => {
    ...
    vi.stubEnv("OMQ_TELEGRAM_BOT_TOKEN", "123:tg-env");
    vi.stubEnv("OMQ_TELEGRAM_CHAT_ID", "tg-chat-env");
  ...
  it("allows OMQ_NOTIFY_TMUX_TAIL_LINES to override file config", () => {
    vi.mocked(existsSync).mockReturnValue(true);
    vi.mocked(readFileSync).mockReturnValue(
      JSON.stringify({
        notifications: {
    ...
    vi.stubEnv("OMQ_NOTIFY_TMUX_TAIL_LINES", "34");
  ...
  it("file mention takes precedence over env mention", () => {
    ...
    vi.stubEnv("OMQ_DISCORD_MENTION", "<@11111111111111111>");
  ...
  it("merges env slack into file config that lacks it", () => {
    ...
    vi.stubEnv("OMQ_SLACK_WEBHOOK_URL", "https://hooks.slack.com/services/env-slack");
  ...
  it("file slack webhookUrl takes precedence over env", () => {
    ...
    vi.stubEnv("OMQ_SLACK_WEBHOOK_URL", "https://hooks.slack.com/services/env-url");
  ...
  it("file slack mention takes precedence over env slack mention", () => {
    ...
    vi.stubEnv("OMQ_SLACK_MENTION", "<@U9999999999>");
