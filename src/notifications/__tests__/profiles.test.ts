...
describe("getNotificationConfig - named profiles", () => {
  ...
  it("returns named profile when OMQ_NOTIFY_PROFILE env var is set", () => {
    vi.stubEnv("OMQ_NOTIFY_PROFILE", "ops");
    vi.mocked(existsSync).mockReturnValue(true);
    vi.mocked(readFileSync).mockReturnValue(
      JSON.stringify({
        notifications: {
  ...
  it("profileName argument takes precedence over OMQ_NOTIFY_PROFILE env var", () => {
    vi.stubEnv("OMQ_NOTIFY_PROFILE", "env-profile");
    vi.mocked(existsSync).mockReturnValue(true);
    vi.mocked(readFileSync).mockReturnValue(
      JSON.stringify({
        notificationProfiles: {
  ...
  it("falls back to default when profile env var set but no profiles exist", () => {
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
    vi.stubEnv("OMQ_NOTIFY_PROFILE", "missing");
    vi.mocked(existsSync).mockReturnValue(true);
    vi.mocked(readFileSync).mockReturnValue(
      JSON.stringify({
        notifications: {
  ...
  it("merges env platforms into profile config", () => {
    vi.stubEnv("OMQ_TELEGRAM_BOT_TOKEN", "env-tg-token");
    vi.stubEnv("OMQ_TELEGRAM_CHAT_ID", "env-tg-chat");
    vi.mocked(existsSync).mockReturnValue(true);
    vi.mocked(readFileSync).mockReturnValue(
      JSON.stringify({
        notificationProfiles: {
