/**
 * Tests for hook notification config reader (omq_config.hook.json).
 *
 * Covers:
 * - File missing → null
 * - File disabled → null
 ...
 * - OMQ_HOOK_CONFIG env var override
 */
...
const TEST_DIR = join(tmpdir(), `omq-hook-config-test-${process.pid}`);
const TEST_CONFIG_PATH = join(TEST_DIR, "omq_config.hook.json");
...
describe("hook-config reader", () => {
  beforeEach(() => {
    resetHookConfigCache();
    vi.stubEnv("OMQ_HOOK_CONFIG", TEST_CONFIG_PATH);
  });
  ...
  it("returns null when file does not exist", () => {
    vi.stubEnv("OMQ_HOOK_CONFIG", join(TEST_DIR, "nonexistent.json"));
  ...
  it("OMQ_HOOK_CONFIG env var overrides default path", () => {
    ...
    vi.stubEnv("OMQ_HOOK_CONFIG", altPath);
    resetHookConfigCache();
    const config = getHookConfig();
