 ...
 *
 * Covers:
 * - Telegram requires OMQ_TELEGRAM=1 to be included
 * - Discord and discord-bot require OMQ_DISCORD=1 to be included
 * - Slack requires OMQ_SLACK=1 to be included
 * - Webhook requires OMQ_WEBHOOK=1 to be included
...
describe('platform gating via getEnabledPlatforms', () => {
  beforeEach(() => {
    ...
    vi.stubEnv('OMQ_TELEGRAM', '');
    vi.stubEnv('OMQ_DISCORD', '');
    vi.stubEnv('OMQ_SLACK', '');
    vi.stubEnv('OMQ_WEBHOOK', '');
  });
  ...
  it('excludes telegram when OMQ_TELEGRAM is not set', () => {
    vi.stubEnv('OMQ_TELEGRAM', '');
    const platforms = getEnabledPlatforms(makeFullConfig(), 'session-end');
    expect(platforms).not.toContain('telegram');
  });

  it('includes telegram when OMQ_TELEGRAM=1', () => {
    vi.stubEnv('OMQ_TELEGRAM', '1');
    const platforms = getEnabledPlatforms(makeFullConfig(), 'session-end');
  ...
  it('excludes discord when OMQ_DISCORD is not set', () => {
    vi.stubEnv('OMQ_DISCORD', '');
    const platforms = getEnabledPlatforms(makeFullConfig(), 'session-end');
    expect(platforms).not.toContain('discord');
  });

  it('excludes discord-bot when OMQ_DISCORD is not set', () => {
    vi.stubEnv('OMQ_DISCORD', '');
    const platforms = getEnabledPlatforms(makeFullConfig(), 'session-end');
    expect(platforms).not.toContain('discord-bot');
  });

  it('includes discord when OMQ_DISCORD=1', () => {
    vi.stubEnv('OMQ_DISCORD', '1');
    const platforms = getEnabledPlatforms(makeFullConfig(), 'session-end');
    expect(platforms).toContain('discord');
  });

  it('includes discord-bot when OMQ_DISCORD=1', () => {
    vi.stubEnv('OMQ_DISCORD', '1');
    const platforms = getEnabledPlatforms(makeFullConfig(), 'session-end');
  ...
  it('excludes slack when OMQ_SLACK is not set', () => {
    vi.stubEnv('OMQ_SLACK', '');
    const platforms = getEnabledPlatforms(makeFullConfig(), 'session-end');
    expect(platforms).not.toContain('slack');
  });

  it('includes slack when OMQ_SLACK=1', () => {
    vi.stubEnv('OMQ_SLACK', '1');
    const platforms = getEnabledPlatforms(makeFullConfig(), 'session-end');
  ...
  it('excludes webhook when OMQ_WEBHOOK is not set', () => {
    vi.stubEnv('OMQ_WEBHOOK', '');
    const platforms = getEnabledPlatforms(makeFullConfig(), 'session-end');
    expect(platforms).not.toContain('webhook');
  });

  it('includes webhook when OMQ_WEBHOOK=1', () => {
    vi.stubEnv('OMQ_WEBHOOK', '1');
    const platforms = getEnabledPlatforms(makeFullConfig(), 'session-end');
