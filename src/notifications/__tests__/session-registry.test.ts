...
describe("session-registry", () => {
  beforeEach(() => {
    ...
    // isolated from the real ~/.omq/state and from other parallel test runs.
    testDir = mkdtempSync(join(tmpdir(), "omq-session-registry-test-"));
    process.env["OMQ_TEST_REGISTRY_DIR"] = testDir;
    REGISTRY_PATH = join(testDir, "reply-session-registry.jsonl");
    LOCK_PATH = join(testDir, "reply-session-registry.lock");
  });

  afterEach(() => {
    delete process.env["OMQ_TEST_REGISTRY_DIR"];
