...
describe("formatAgentCall", () => {
  ...
  it("should include agent type when provided", () => {
    ...
    expect(result).toContain("**Type:** `oh-my-qoder:executor`");
  });
...
describe("parseTmuxTail noise filters", () => {
  ...
  it("drops OMQ HUD versioned status lines", () => {
    expect(
      parseTmuxTail("[OMQ#4.4.5] | thinking | session:510m | ctx:61% | 🔧57"),
    ).toBe("");
  });

  it("drops unversioned OMQ HUD lines", () => {
    expect(parseTmuxTail("[OMQ] | session:5m")).toBe("");
  });
  ...
  it("returns empty string when all lines are noise types", () => {
    const input = [
      "────────────────────────",
      "[OMQ#4.4.5] | thinking | session:510m",
