...
const CONFIG_FILE = join(getQoderConfigDir(), ".omq-config.json");
const DEFAULT_TMUX_TAIL_LINES = 15;


/**
 * Read raw config from .omq-config.json
 */
...
export function buildConfigFromEnv(): NotificationConfig | null {
  ...
  // Telegram (support both OMQ_TELEGRAM_BOT_TOKEN and OMQ_TELEGRAM_NOTIFIER_BOT_TOKEN)
  const telegramToken =
  ...
  if (slackWebhook) {
    config.slack = {
      ...
      mention: validateSlackMention(process.env.OMQ_SLACK_MENTION),
    };
    hasAnyPlatform = true;
  }

  // Slack Bot (app token + bot token + channel)
  const slackBotToken = process.env.OMQ_SLACK_BOT_TOKEN;
  const slackBotChannel = process.env.OMQ_SLACK_BOT_CHANNEL;
  if (slackBotToken && slackBotChannel) {
    config["slack-bot"] = {
      enabled: true,
      appToken: process.env.OMQ_SLACK_APP_TOKEN,
...
function applyEnvMerge(config: NotificationConfig): NotificationConfig {
  ...
  const envSlackMention = validateSlackMention(process.env.OMQ_SLACK_MENTION);
  if (envSlackMention) {
 ...
 *
 * Priority: OMQ_NOTIFY_VERBOSITY env var > config.verbosity > "session" default.
...
export function getVerbosity(config: NotificationConfig): VerbosityLevel {
  const envValue = process.env.OMQ_NOTIFY_VERBOSITY;
 ...
 *
 * Priority: OMQ_NOTIFY_TMUX_TAIL_LINES env var > config.tmuxTailLines > 15 default.
...
export function getTmuxTailLines(config: NotificationConfig): number {
  const envValue = Number.parseInt(process.env.OMQ_NOTIFY_TMUX_TAIL_LINES ?? "", 10);
 ...
 *
 * When a profile name is provided (or set via OMQ_NOTIFY_PROFILE env var),
 ...
 *
 * Reads from .omq-config.json, looking for the `notifications` key.
 ...
 *
 * @param profileName - Optional profile name (overrides OMQ_NOTIFY_PROFILE env var)
 */
export function getNotificationConfig(profileName?: string): NotificationConfig | null {
  const raw = readRawConfig();
  const effectiveProfile = profileName || process.env.OMQ_NOTIFY_PROFILE;
  ...
  // Priority 2: Explicit notifications config in .omq-config.json
  if (raw) {
 ...
 *   --telegram  -> OMQ_TELEGRAM=1
 *   --discord   -> OMQ_DISCORD=1
 *   --slack     -> OMQ_SLACK=1
 *   --webhook   -> OMQ_WEBHOOK=1
 */
function isPlatformActivated(platform: NotificationPlatform): boolean {
  if (platform === "telegram") return process.env.OMQ_TELEGRAM === "1";
  if (platform === "discord" || platform === "discord-bot")
    return process.env.OMQ_DISCORD === "1";
  if (platform === "slack" || platform === "slack-bot")
    return process.env.OMQ_SLACK === "1";
  if (platform === "webhook") return process.env.OMQ_WEBHOOK === "1";
  return false;
}
 ...
 *
 * Reads from .omq-config.json notifications.reply section.
 * Environment variables override config file values:
 * - OMQ_REPLY_ENABLED: enable reply listening (default: false)
 * - OMQ_REPLY_POLL_INTERVAL_MS: polling interval in ms (default: 3000)
 * - OMQ_REPLY_RATE_LIMIT: max messages per minute (default: 10)
 * - OMQ_REPLY_DISCORD_USER_IDS: comma-separated authorized Discord user IDs
 * - OMQ_REPLY_INCLUDE_PREFIX: include visual prefix (default: true)
 *
...
export function getReplyConfig(): import("./types.js").ReplyConfig | null {
  ...
  const authorizedSlackUserIds = parseSlackUserIds(
    process.env.OMQ_REPLY_SLACK_USER_IDS,
    replyRaw?.authorizedSlackUserIds,
  );

  return {
    enabled: true,
    pollIntervalMs: parseIntSafe(process.env.OMQ_REPLY_POLL_INTERVAL_MS) ?? replyRaw?.pollIntervalMs ?? 3000,
    maxMessageLength: replyRaw?.maxMessageLength ?? 500,
    rateLimitPerMinute: parseIntSafe(process.env.OMQ_REPLY_RATE_LIMIT) ?? replyRaw?.rateLimitPerMinute ?? 10,
...
const LEGACY_OPENCLAW_CONFIG = join(getQoderConfigDir(), "omq_config.openclaw.json");
...
/**
 * Read custom integrations configuration from .omq-config.json.
 */
