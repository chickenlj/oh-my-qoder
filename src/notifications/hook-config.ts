 ...
 *
 * Reads omq_config.hook.json for user-customizable message templates.
...
const DEFAULT_CONFIG_PATH = join(getQoderConfigDir(), "omq_config.hook.json");
 ...
 * - File path overridable via OMQ_HOOK_CONFIG env var (for testing)
 */
export function getHookConfig(): HookNotificationConfig | null {
  if (cachedConfig !== undefined) return cachedConfig;

  const configPath = process.env.OMQ_HOOK_CONFIG || DEFAULT_CONFIG_PATH;
