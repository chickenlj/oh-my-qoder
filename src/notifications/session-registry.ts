 ...
 *
 * Registry location: XDG-aware global OMQ state (legacy ~/.omq/state fallback for reads)
...
import { getGlobalOmqStateCandidates, getGlobalOmqStateRoot } from '../utils/paths.js';
...
function getRegistryStateDir(): string {
  return process.env['OMQ_TEST_REGISTRY_DIR'] ?? getGlobalOmqStateRoot();
}
...
function getRegistryReadPaths(): string[] {
  ...
  return getGlobalOmqStateCandidates('reply-session-registry.jsonl');
}
