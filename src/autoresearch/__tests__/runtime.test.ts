...
describe('autoresearch runtime', () => {
  ...
  it('prepares runtime artifacts and persists autoresearch mode state', async () => {
    ...
    try {
      ...
      const worktreePath = join(repo, '..', `${repo.split('/').pop()}.omq-worktrees`, 'autoresearch-missions-demo-20260314t000000z');
      ...
      expect(state?.mission_spec_file).toBe(join(repo, '.omq', 'autoresearch', 'missions-demo', 'mission.md'));
      expect(state?.evaluator_reference_file).toBe(join(repo, '.omq', 'autoresearch', 'missions-demo', 'evaluator.json'));
  ...
  it('materializes canonical mission artifacts and markdown decision log paths', async () => {
    ...
    try {
      const contract = await makeContract(repo);
      const worktreePath = join(repo, '..', `${repo.split('/').pop()}.omq-worktrees`, 'autoresearch-missions-demo-20260314t000500z');
...
describe('autoresearch parity decisions', () => {
  it('keeps improved candidates and resets discarded candidates back to the last kept commit', async () => {
    ...
    try {
      const contract = await makeContract(repo);
      const worktreePath = join(repo, '..', `${repo.split('/').pop()}.omq-worktrees`, 'autoresearch-missions-demo-20260314t010000z');
      ...
      const decisionLog = await readFile(
        join(repo, '.omq', 'autoresearch', 'missions-demo', 'runs', runtime.runId, 'decision-log.md'),
        'utf-8',
      );
      ...
      const evaluationOne = JSON.parse(await readFile(
        join(repo, '.omq', 'autoresearch', 'missions-demo', 'runs', runtime.runId, 'evaluations', 'iteration-0001.json'),
        'utf-8',
