...
export const DEPRECATED_MCP_PROVIDER_WARNING =
  '[OMQ] Codex/Gemini MCP delegation is deprecated. Use /team to coordinate CLI workers instead.';
...
function resolveExplicitTool(
  ...
  // Only 'Task' is supported - explicit tool invocation always uses Qwen
  return {
    provider: 'qwen',
    tool: 'Task',
...
function resolveFromConfig(
  ...
  // fallback-chain evidence while routing to an executable Qwen Task target.
  // External model names are not valid Qwen subagent roles, so route.model
  ...
  if (isDeprecatedMcpProvider(provider)) {
    ...
    const qwenAgent = route.agentType || agentRole;
    const modelEvidence = route.model ? `; ignored external model "${route.model}"` : '';
    return {
      provider: 'qwen',
      tool: 'Task',
      agentOrModel: qwenAgent,
      reason: `Configured routing for role "${agentRole}" (deprecated provider "${provider}", falling back to Qwen Task${modelEvidence})`,
      fallbackChain,
    };
  }

  // Only qwen → Task is valid; correct any mismatch
  if (tool !== 'Task') {
...
function resolveDefault(
  ...
  if (defaultAgent) {
    return {
      ...
      reason: `Default heuristic: role "${agentRole}" → Qwen subagent "${defaultAgent}"`,
    };
  }

  // Fall back to default provider or qwen
  const defaultProvider = config?.defaultProvider || 'qwen';

  if (isDeprecatedMcpProvider(defaultProvider)) {
    console.warn(DEPRECATED_MCP_PROVIDER_WARNING);
  }

  // Default to qwen Task (codex/gemini default providers fall back to qwen)
  return {
    provider: 'qwen',
    tool: 'Task',
    agentOrModel: agentRole,
    reason: `Fallback to Qwen Task for role "${agentRole}"`,
  };
}
