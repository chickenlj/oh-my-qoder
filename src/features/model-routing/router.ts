 ...
 * @returns The recommended model type ('low', 'medium', or 'high')
 */
export function getModelForTask(
  agentType: string,
  taskPrompt: string,
  config: Partial<RoutingConfig> = {}
): { model: 'low' | 'medium' | 'high'; tier: ComplexityTier; reason: string } {
  // All agents are adaptive based on task complexity
  ...
  return {
    model: decision.modelType as 'low' | 'medium' | 'high',
    tier: decision.tier,
