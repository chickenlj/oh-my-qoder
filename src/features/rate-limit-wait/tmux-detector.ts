 ...
 * tmux Detector
 *
 * Detects Qoder CLI sessions running in tmux panes and identifies
...
/** Patterns that indicate Qoder CLI is running */
...
/**
 * Analyze pane content to determine if it shows a rate-limited Qoder CLI session
 */
export function analyzePaneContent(content: string): PaneAnalysisResult {
  ...
  // Check for Qoder CLI indicators
...
/**
 * Scan all tmux panes for blocked Qoder CLI sessions.
 *
...
export function formatBlockedPanesSummary(blockedPanes: BlockedPane[]): string {
  if (blockedPanes.length === 0) {
    return 'No blocked Qoder CLI sessions detected.';
  }

  const lines: string[] = [
    `Found ${blockedPanes.length} blocked Qoder CLI session(s):`,
    '',
  ];
