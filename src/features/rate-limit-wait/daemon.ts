...
const DEFAULT_CONFIG: Required<DaemonConfig> = {
  ...
  verbose: false,
  stateFilePath: getGlobalOmqStatePath('rate-limit-daemon.json'),
  pidFilePath: getGlobalOmqStatePath('rate-limit-daemon.pid'),
  logFilePath: getGlobalOmqStatePath('rate-limit-daemon.log'),
};
...
export function startDaemon(config?: DaemonConfig): DaemonResponse {
  ...
  const daemonScript = `
    import(${JSON.stringify(moduleUrl)}).then(async ({ pollLoopWithConfigFile }) => {
      await pollLoopWithConfigFile(process.env.OMQ_DAEMON_CONFIG_FILE);
  ...
  try {
    ...
    const daemonEnv = {
      ...
      OMQ_DAEMON_CONFIG_FILE: configPath,
    };
...
export async function runDaemonForeground(config?: DaemonConfig): Promise<void> {
  ...
  // Check if already running
  if (isDaemonRunning(cfg)) {
    console.error('Daemon is already running. Use "omq wait daemon stop" first.');
    process.exit(1);
  }
