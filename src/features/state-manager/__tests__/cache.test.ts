...
const { TEST_STATE_DIR, TEST_WORKTREE_ROOT } = vi.hoisted(() => ({
  TEST_STATE_DIR: '/tmp/omq-cache-test-state',
  TEST_WORKTREE_ROOT: '/tmp/omq-cache-test-worktree',
}));
...
vi.mock('../../../lib/worktree-paths.js', () => ({
  ...
  getOmqRoot: (dir?: string) => `${dir ?? TEST_WORKTREE_ROOT}/.omq`,
}));
...
describe('state-manager cache', () => {
  ...
  function writeLegacyStateToDisk(name: string, data: unknown) {
    const legacyDir = path.join(TEST_WORKTREE_ROOT, '.omq', 'state');
...
describe('cleanupStaleStates', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join('/tmp', 'omq-cleanup-test-'));
    const stateDir = path.join(tmpDir, '.omq', 'state');
  ...
  function readStateFile(name: string) {
    const filePath = path.join(tmpDir, '.omq', 'state', `${name}.json`);
