 ...
 * local (.omq/state/) and global (XDG-aware user OMQ state with legacy ~/.omq/state fallback) locations.
 */
...
export enum StateLocation {
  /** Local project state: .omq/state/{name}.json */
  LOCAL = 'local',
  /** Global user state: XDG-aware OMQ state path with legacy ~/.omq/state fallback on reads */
  GLOBAL = 'global'
}
