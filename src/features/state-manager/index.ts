 ...
 * - Local state: .omq/state/{name}.json
 * - Global state: XDG-aware user OMQ state with legacy ~/.omq/state fallback
 *
 * Features:
...
function getLocalStateDir(): string {
  return path.join(validateWorkingDirectory(), OmqPaths.STATE);
}
/**
 ...
 */
const GLOBAL_STATE_DIR = getGlobalOmqStateRoot();
...
function warnStateReadFailure(filePath: string, error: unknown): void {
  console.warn(`Failed to read state from ${filePath}:`, error);
}
...
export function readState<T = StateData>(
  ...
  if (fs.existsSync(standardPath)) {
    try {
      ...
      warnStateReadFailure(standardPath, error);
    }
  }
 ...
 * Clear state file.
 *
 * Removes the state file from standard location.
...
export function migrateState(
  _name: string,
  _location: StateLocation = StateLocation.LOCAL,
): StateMigrationResult {
...
export function cleanupStaleStates(
  ...
  const stateDir = directory
    ? path.join(getOmqRoot(directory), "state")
    : getLocalStateDir();
  ...
  // Scan top-level state files (.omq/state/*.json)
  scanDir(stateDir);

  // Scan session directories (.omq/state/sessions/*/*.json)
