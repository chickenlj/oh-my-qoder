 ...
 * ${CLAUDE_CONFIG_DIR:-$HOME/.qoder} pattern.
 */
function findHardcodedHomeClaude(filePath: string): { line: number; text: string }[] {
  ...
  for (const block of blocks) {
    ...
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i];
      // Match $HOME/.qoder that is NOT inside ${CLAUDE_CONFIG_DIR:-$HOME/.qoder}
      if (/\$HOME\/\.qoder/.test(line) && !/\$\{CLAUDE_CONFIG_DIR:-\$HOME\/\.qoder\}/.test(line)) {
        violations.push({
 ...
 * without portable notation like [$CLAUDE_CONFIG_DIR|~/.qoder].
...
function findHardcodedTildeClaude(filePath: string): { line: number; text: string }[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Allow: portable notation [$CLAUDE_CONFIG_DIR|~/.qoder]
    if (/\[\$CLAUDE_CONFIG_DIR\|~\/\.qoder\]/.test(line)) continue;
    ...
    (_label, filePath) => {
      ...
      if (violations.length > 0) {
        const details = violations
          ...
          .join('\n');
        expect.fail(
          `Found $HOME/.qoder without CLAUDE_CONFIG_DIR fallback:\n${details}\n` +
          `Replace with: \${CLAUDE_CONFIG_DIR:-$HOME/.qoder}`
        );
      }
    },
  );
...
describe('skill markdown prose must not use raw ~/.qoder (Contract 6, issue #2155 §16)', () => {
  ...
  // To reduce the baseline: fix the skill prose to use [$CLAUDE_CONFIG_DIR|~/.qoder] notation,
  ...
  const KNOWN_VIOLATION_BASELINE: Record<string, number> = {
    ...
    'skills/omq-setup/phases/01-install-claude-md.md': 4,
    ...
    (label, filePath) => {
      ...
      if (violations.length > baseline) {
        ...
        expect.fail(
          ...
          `Replace with: [$CLAUDE_CONFIG_DIR|~/.qoder] or use \${CLAUDE_CONFIG_DIR:-$HOME/.qoder} in code`
        );
      }
    },
  );
