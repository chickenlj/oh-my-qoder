 ...
 *
 * Companion to artifacts.test.ts. Verifies that when a .omq-workspace marker
 * exists in a parent directory, ultragoal artifacts are written to the
 * workspace anchor's .omq/ instead of the sub-repo's .omq/.
 */
...
describe('ultragoal artifacts — multi-repo workspace anchor', () => {
  it('writes artifacts to workspace anchor .omq/ when .omq-workspace marker is in a parent dir', async () => {
    ...
    try {
      ...
      expect(existsSync(join(workspaceRoot, '.omq', 'ultragoal', 'goals.json'))).toBe(true);
  ...
  it('sibling sub-repos share one workspace .omq/ when rooted at the same .omq-workspace', async () => {
    ...
    try {
      ...
      // Both plans land under the single workspace .omq/
      expect(existsSync(join(workspaceRoot, '.omq', 'ultragoal', 'plans', 'plan-a', 'goals.json'))).toBe(true);
      expect(existsSync(join(workspaceRoot, '.omq', 'ultragoal', 'plans', 'plan-b', 'goals.json'))).toBe(true);

      // Sub-repos must not have their own .omq/ultragoal
      expect(existsSync(join(repoA, '.omq', 'ultragoal'))).toBe(false);
      expect(existsSync(join(repoB, '.omq', 'ultragoal'))).toBe(false);
  ...
  it('full lifecycle (start → checkpoint) resolves through workspace anchor', async () => {
    ...
    try {
      ...
      const ledger = await readFile(join(workspaceRoot, '.omq', 'ultragoal', 'ledger.jsonl'), 'utf-8');
