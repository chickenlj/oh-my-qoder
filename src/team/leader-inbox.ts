...
//
// Leader inbox path: .omq/state/team/{sanitizedTeam}/leader/inbox.md
...
export function leaderInboxPath(teamName: string, cwd: string): string {
  ...
  return join(cwd, `.omq/state/team/${safe}/leader/inbox.md`);
}

/**
 * Ensures the leader inbox directory and seed file exist.
 * Creates .omq/state/team/{team}/leader/ and seeds inbox.md with a header banner.
...
export function extendLeaderBootstrapPrompt(teamName: string): string {
  const safe = sanitizeName(teamName);
  const path = `.omq/state/team/${safe}/leader/inbox.md`;
