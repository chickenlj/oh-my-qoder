 ...
 * Path: .omq/reports/team-{teamName}-{timestamp}.md
...
export function saveTeamReport(
  ...
  const dir = join(getOmqRoot(workingDirectory), 'reports');
  ensureDirWithMode(dir);
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
  const filePath = join(dir, `team-${teamName}-${timestamp}.md`);
  // filePath lives under getOmqRoot(...)/reports, which in a .omq-workspace
