...
describe('team summary worktree metadata', () => {
  it('surfaces workspace and worker worktree contract fields', async () => {
    const cwd = mkdtempSync(join(tmpdir(), 'omq-summary-worktree-'));
    const teamName = 'summary-team';
    const teamRoot = join(cwd, '.omq', 'state', 'team', teamName);
    const teamStateRoot = join(cwd, '.omq', 'state', 'team', teamName);
    const worktreePath = join(cwd, '.omq', 'team', teamName, 'worktrees', 'worker-1');
    try {
      ...
      writeFileSync(join(teamRoot, 'config.json'), JSON.stringify({
        ...
        workers: [{
          ...
          worktree_branch: 'omq-team/summary-team/worker-1',
          worktree_detached: false,
          worktree_created: true,
          team_state_root: teamStateRoot,
        }],
        created_at: new Date().toISOString(),
        tmux_session: 'omq-summary-team',
        next_task_id: 1,
        leader_cwd: cwd,
        team_state_root: teamStateRoot,
