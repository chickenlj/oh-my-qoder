...
describe('WorkerInfo.worker_cli round-trip', () => {
  ...
  it('preserves worker_cli=claude through JSON serialization', () => {
    ...
    expect(out.worker_cli).toBe('qwen');
  ...
  it('preserves output_file alongside worker_cli', () => {
    const w: WorkerInfo = {
      ...
      worker_cli: 'codex',
      assigned_tasks: ['7'],
      output_file: '.omq/state/team/x/workers/worker-4/verdict.json',
    };
    const out = roundtrip(w);
    expect(out.worker_cli).toBe('codex');
    expect(out.output_file).toBe('.omq/state/team/x/workers/worker-4/verdict.json');
  });
});
