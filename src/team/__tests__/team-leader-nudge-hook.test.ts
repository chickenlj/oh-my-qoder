...
describe('team leader nudge hook', () => {
  ...
  beforeEach(async () => {
    cwd = await mkdtemp(join(tmpdir(), 'omq-team-leader-nudge-'));
  });
  ...
  async function seedTeamState(options: {
    ...
    const teamRoot = '.omq/state/team/demo-team';
  ...
  it('nudges leader to reuse current team when workers are idle with active tasks', async () => {
    ...
    const eventsRaw = await readFile(join(cwd, '.omq', 'state', 'team', 'demo-team', 'events.jsonl'), 'utf-8');
