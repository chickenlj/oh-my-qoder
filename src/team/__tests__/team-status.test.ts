...
beforeEach(() => {
  ...
  mkdirSync(join(WORK_DIR, '.omq', 'state'), { recursive: true });
});
...
function writeWorkerRegistry(workers: McpWorkerMember[]): void {
  const registryPath = join(WORK_DIR, '.omq', 'state', 'team-mcp-workers.json');
...
function writeHeartbeatFile(data: HeartbeatData): void {
  const hbPath = join(WORK_DIR, '.omq', 'state', 'team-bridge', TEST_TEAM, `${data.workerName}.heartbeat.json`);
...
function makeWorker(name: string, provider: 'codex' | 'gemini' = 'codex'): McpWorkerMember {
  return {
    ...
    model: 'test-model',
    joinedAt: Date.now(),
    tmuxPaneId: `omq-team-${TEST_TEAM}-${name}`,
    cwd: WORK_DIR,
    backendType: 'tmux',
    subscriptions: [],
  };
