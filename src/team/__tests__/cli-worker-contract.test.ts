...
describe('cli-worker-contract', () => {
  describe('shouldInjectContract', () => {
    ...
    it('returns false for claude workers regardless of role', () => {
      expect(shouldInjectContract('critic', 'qwen')).toBe(false);
      expect(shouldInjectContract('code-reviewer', 'qwen')).toBe(false);
    });
  ...
  describe('cliWorkerOutputFilePath', () => {
    it('joins team state root + worker into the conventional path', () => {
      const p = cliWorkerOutputFilePath('/repo/.omq/state/team/foo', 'worker-2');
      expect(p).toBe('/repo/.omq/state/team/foo/workers/worker-2/verdict.json');
    });

    it('normalizes windows backslashes to forward slashes', () => {
      const p = cliWorkerOutputFilePath('C:\\proj\\.omq\\state\\team\\foo', 'worker-1');
      expect(p).toBe('C:/proj/.omq/state/team/foo/workers/worker-1/verdict.json');
    });
  });
