...
describe('team leader nudge hook logging', () => {
  ...
  beforeEach(async () => {
    cwd = await mkdtemp(join(tmpdir(), 'omq-team-leader-nudge-logging-'));
  ...
  it('logs appendTeamEvent persistence failures without failing the nudge', async () => {
    await writeJson('.omq/state/team/demo-team/config.json', {
      workers: [{ name: 'worker-1' }],
      leader_pane_id: '%1',
    });
    await writeJson('.omq/state/team/demo-team/workers/worker-1/status.json', {
      state: 'idle',
      updated_at: new Date().toISOString(),
    });
    await writeJson('.omq/state/team/demo-team/workers/worker-1/heartbeat.json', {
      alive: true,
      last_turn_at: new Date().toISOString(),
    });
    await writeJson('.omq/state/team/demo-team/tasks/task-1.json', {
      status: 'pending',
    });
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      '[omq] hooks.team-leader-nudge maybeNudgeLeader persistence failed: event write failed',
    );
  });
});
