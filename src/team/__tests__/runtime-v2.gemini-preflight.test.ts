...
const modelContractMocks = vi.hoisted(() => ({
  buildWorkerArgv: vi.fn((agentType?: string, config?: { resolvedBinaryPath?: string }) => [config?.resolvedBinaryPath ?? agentType ?? 'qwen']),
  resolveValidatedBinaryPath: vi.fn((agentType?: string) => {
    ...
    return `/usr/bin/${agentType ?? 'qwen'}`;
  }),
  getContract: vi.fn((agentType?: string) => ({ binary: agentType ?? 'qwen' })),
