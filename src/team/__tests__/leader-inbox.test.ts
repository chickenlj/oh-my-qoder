...
describe('leaderInboxPath', () => {
  it('returns correct path under cwd', () => {
    ...
    expect(p).toBe(join(TEST_CWD, '.omq/state/team/my-team/leader/inbox.md'));
  });
...
describe('ensureLeaderInbox', () => {
  it('creates the leader directory', async () => {
    ...
    const dir = join(TEST_CWD, '.omq/state/team/my-team/leader');
...
describe('appendToLeaderInbox', () => {
  ...
  it('creates parent directories if missing', async () => {
    ...
    const freshCwd = join(tmpdir(), `omq-test-leader-inbox-fresh-${process.pid}`);
    try {
...
describe('extendLeaderBootstrapPrompt', () => {
  it('contains the canonical leader inbox path', () => {
    ...
    expect(prompt).toContain('.omq/state/team/my-team/leader/inbox.md');
  });
  ...
  it('sanitizes team name in the path', () => {
    ...
    const pathMatch = prompt.match(/\.omq\/state\/team\/([^/]+)\/leader\/inbox\.md/);
