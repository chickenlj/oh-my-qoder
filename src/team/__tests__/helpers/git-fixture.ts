...
//   - Worker branches are omq-team/{teamName}/{workerName} (matching getBranchName)
//   - Leader branch must NOT be main/master (M3 hardening) — defaults to 'omq-team-test-leader'
//   - State dir (.omq/...) is created inside the repo root so orchestrator paths resolve
...
export async function createGitFixture(opts: CreateGitFixtureOpts): Promise<GitFixture> {
  const {
    workerCount,
    leaderBranchName = 'omq-team-test-leader',
    teamName = 'test-team',
  ...
  const tmpBase = mkdtempSync(join(tmpdir(), 'omq-test-'));
  ...
  const workersDir = join(repoRoot, '.omq', 'team', teamName, 'worktrees');
  mkdirSync(workersDir, { recursive: true });

  const workers: WorkerFixture[] = [];
  for (let i = 0; i < workerCount; i++) {
    const workerName = `worker-${i + 1}`;
    const branch = `omq-team/${teamName}/${workerName}`;
  ...
  const worktreesMetaDir = join(repoRoot, '.omq', 'state', 'team', teamName);
  ...
  return {
    ...
    async cleanup(): Promise<void> {
      ...
      try {
        ...
        const mergerPath = join(repoRoot, '.omq', 'team', teamName, 'merger');
...
export function orchestratorEventLogPath(repoRoot: string, teamName: string): string {
  return join(repoRoot, '.omq', 'state', 'team', teamName, 'orchestrator-events.jsonl');
}
