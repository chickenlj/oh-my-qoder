...
const BASE_MERGE_ARGS = {
  workerName: 'writer',
  workerBranch: 'omq-team/test-team/writer',
  leaderBranch: 'omq-team/test-team-leader',
...
const BASE_REBASE_ARGS = {
  ...
  conflictingFiles: CONFLICTS,
  baseSha: 'def5678',
  worktreePath: '/repo/.omq/team/test-team/worktrees/writer',
...
describe('formatMergeConflictForLeader', () => {
  it('matches snapshot', () => {
    ...
    expect(result).toMatchInlineSnapshot(`
      "### Merge conflict: writer → omq-team/test-team-leader

      **Worker branch:** \`omq-team/test-team/writer\`
      **Leader branch:** \`omq-team/test-team-leader\`
      **Merge base:** \`abc1234\`
      ...
      **Leader: choose strategy.** To resolve, run:
      ...
      \`\`\`sh
      git checkout omq-team/test-team-leader && git merge --no-ff omq-team/test-team/writer
  ...
  it('includes resolution recipe with correct branches', () => {
    const result = formatMergeConflictForLeader(BASE_MERGE_ARGS);
    expect(result).toContain('git checkout omq-team/test-team-leader && git merge --no-ff omq-team/test-team/writer');
  });
...
describe('formatRebaseConflictForWorker', () => {
  it('matches snapshot', () => {
    ...
    expect(result).toMatchInlineSnapshot(`
      "### Rebase conflict: writer onto omq-team/test-team-leader

      **Worker branch:** \`omq-team/test-team/writer\`
      **Base branch:** \`omq-team/test-team-leader\`
      **Base SHA:** \`def5678\`
      **Worktree:** \`/repo/.omq/team/test-team/worktrees/writer\`
      **Observed at:** 2001-09-09T01:46:40.000Z
  ...
  it('includes worktree path', () => {
    const result = formatRebaseConflictForWorker(BASE_REBASE_ARGS);
    expect(result).toContain('/repo/.omq/team/test-team/worktrees/writer');
  });
...
describe('deliverMergeConflictToLeader', () => {
  it('writes to .omq/state/team/{team}/leader/inbox.md', async () => {
    ...
    const expectedPath = join(TEST_CWD, `.omq/state/team/${TEST_TEAM}/leader/inbox.md`);
...
describe('deliverRebaseConflictToWorker', () => {
  it('writes to .omq/state/team/{team}/workers/{worker}/inbox.md', async () => {
    ...
    const expectedPath = join(TEST_CWD, `.omq/state/team/${TEST_TEAM}/workers/writer/inbox.md`);
    expect(existsSync(expectedPath)).toBe(true);
    const content = readFileSync(expectedPath, 'utf-8');
    expect(content).toContain(message);
  });
