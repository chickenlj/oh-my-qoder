...
function getRestartStatePath(workingDirectory: string, teamName: string, workerName: string): string {
  return join(getOmqRoot(workingDirectory), 'state', 'team-bridge', teamName, `${workerName}.restart.json`);
}
...
export function recordRestart(
  ...
  const statePath = getRestartStatePath(workingDirectory, teamName, workerName);
  // statePath lives under getOmqRoot(...)/state/team-bridge, which in a
  // .omq-workspace layout is ABOVE workingDirectory. Validate against the shared
  // team-bridge dir (still catches teamName/workerName traversal) not the sub-repo.
  validateResolvedPath(statePath, join(getOmqRoot(workingDirectory), 'state', 'team-bridge'));

  const dir = join(getOmqRoot(workingDirectory), 'state', 'team-bridge', teamName);
