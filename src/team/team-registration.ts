...
function shadowRegistryPath(workingDirectory: string): string {
  const result = join(getOmqRoot(workingDirectory), 'state', 'team-mcp-workers.json');
  validateResolvedPath(result, join(getOmqRoot(workingDirectory), 'state'));
  return result;
}

function probeResultPath(workingDirectory: string): string {
  return join(getOmqRoot(workingDirectory), 'state', 'config-probe-result.json');
}
 ...
 * - 'shadow': Write member to .omq/state/team-mcp-workers.json (fallback)
 *
