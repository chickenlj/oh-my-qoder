 ...
 * Workers use `omq team api` CLI commands for all lifecycle transitions.
 */
function buildV2TaskInstruction(
  ...
  const failTaskCommand = formatOmqCliInvocation(
...
interface SpawnV2WorkerOptions {
   ...
   * selection (e.g. codex with gpt-5-codex for reviewer, qwen-max for critic).
   */
  model?: string;
  /**
...
async function spawnV2Worker(opts: SpawnV2WorkerOptions): Promise<SpawnV2WorkerResult> {
  ...
  // per-role routing (codex/gemini/grok/cursor/qwen-tier) is honored at spawn time.
  ...
  if (opts.agentType === 'qwen') {
    ...
    // Qoder CLI v2.1.x sometimes swallows the Enter key sent immediately
    ...
    if (!settled) {
      return {
        paneId,
        startupAssigned: false,
        startupFailureReason: 'qwen_startup_evidence_missing',
      };
    }
  }
...
export async function startTeamV2(config: StartTeamV2Config): Promise<TeamRuntimeV2> {
  ...
  if (!resolvedBinaryPaths.qwen) {
    try {
      resolvedBinaryPaths.qwen = resolveValidatedBinaryPath('qwen');
    } catch {
  ...
  await mkdir(join(getOmqRoot(leaderCwd), 'state', 'team', sanitized, 'mailbox'), { recursive: true });
  ...
  for (const { agentType, reason } of missingBinaryReasons) {
    process.stderr.write(
      `[team/runtime-v2] cli_binary_missing:${agentType}: ${reason} — falling back to qwen snapshot (AC-8)\n`,
    );
  ...
  if (unownedTaskIndices.length > 0) {
    ...
    const allocationWorkers: WorkerAllocationInput[] = workerNames.map((name, i) => ({
      ...
      role: config.workerRoles?.[i]
        ?? (agentTypes[i % agentTypes.length] ?? agentTypes[0] ?? 'qwen') as string,
      currentLoad: 0,
    }));
  ...
  try {
    for (let i = 0; i < workerNames.length; i++) {
      ...
      const agentType = (agentTypes[i % agentTypes.length] ?? agentTypes[0] ?? 'qwen') as CliAgentType;
  ...
  try {
    ...
    const fallbackAgent = (agentTypes[workerIndex % agentTypes.length] ?? agentTypes[0] ?? 'qwen') as CliAgentType;
...
export async function resumeTeamV2(
  ...
  try {
    const sessionName = config.tmux_session || `omq-team-${sanitized}`;
...
export async function findActiveTeamsV2(cwd: string): Promise<string[]> {
  const root = join(getOmqRoot(cwd), 'state', 'team');
