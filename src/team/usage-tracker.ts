 ...
 *
 * Storage: append-only JSONL at .omq/logs/team-usage-{team}.jsonl
 */
...
function getUsageLogPath(workingDirectory: string, teamName: string): string {
  return join(getOmqRoot(workingDirectory), 'logs', `team-usage-${teamName}.jsonl`);
}
...
export function recordTaskUsage(
  ...
  const dir = join(getOmqRoot(workingDirectory), 'logs');
  // logPath lives under getOmqRoot(...)/logs, which in a .omq-workspace layout
