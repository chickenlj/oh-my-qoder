...
/**
 * Get the path to the ralphthon PRD file in .omq
 */
...
/**
 * Find ralphthon-prd.json (checks both root and .omq)
 */
export function findRalphthonPrdPath(directory: string): string | null {
  const rootPath = join(directory, PRD_FILENAME);
  if (existsSync(rootPath)) return rootPath;

  const omqPath = getRalphthonPrdPath(directory);
  if (existsSync(omqPath)) return omqPath;
