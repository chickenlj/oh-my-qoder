 ...
 *
 * Reads OpenClaw config from ~/.qoder/omq_config.openclaw.json.
 * Config is cached after first read (env vars don't change during process lifetime).
 * Config file path can be overridden via OMQ_OPENCLAW_CONFIG env var.
 */
...
const CONFIG_FILE = process.env.OMQ_OPENCLAW_CONFIG
  || join(getQoderConfigDir(), "omq_config.openclaw.json");
