...
vi.mock("../../utils/config-dir.js", () => ({
  getQoderConfigDir: vi.fn(() => "/home/user/.qoder"),
}));
...
describe("getOpenClawConfig", () => {
  ...
  it("returns null when OMQ_OPENCLAW is not set", () => {
    vi.stubEnv("OMQ_OPENCLAW", "");
    expect(getOpenClawConfig()).toBeNull();
  });

  it("returns null when OMQ_OPENCLAW is not '1'", () => {
  ...
  it("returns valid config when file exists and OMQ_OPENCLAW=1", () => {
    vi.stubEnv("OMQ_OPENCLAW", "1");
  ...
  it("respects OMQ_OPENCLAW_CONFIG env var for custom config path", () => {
    vi.stubEnv("OMQ_OPENCLAW", "1");
    vi.stubEnv("OMQ_OPENCLAW_CONFIG", "/custom/path/config.json");
