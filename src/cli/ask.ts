...
export const ASK_USAGE = [
  'Usage: omq ask <claude|codex|gemini|grok|cursor> <question or task>',
  '   or: omq ask <claude|codex|gemini|grok|cursor> -p "<prompt>"',
  '   or: omq ask <claude|codex|gemini|grok|cursor> --print "<prompt>"',
  '   or: omq ask <claude|codex|gemini|grok|cursor> --prompt "<prompt>"',
  '   or: omq ask <claude|codex|gemini|grok|cursor> --agent-prompt <role> "<prompt>"',
  '   or: omq ask <claude|codex|gemini|grok|cursor> --agent-prompt=<role> --prompt "<prompt>"',
].join('\n');

const ASK_PROVIDERS = ['qwen', 'codex', 'gemini', 'grok', 'cursor'] as const;
...
export async function askCommand(args: string[]): Promise<void> {
  ...
  if (parsed.provider !== 'qwen' && isExternalLLMDisabled()) {
    throw new Error(
      `[ask] External LLM provider "${parsed.provider}" is blocked by security policy ` +
      `(disableExternalLLM). Only "qwen" is allowed in the current security configuration.`,
    );
  }
