/**
 * Interop CLI Command - Split-pane tmux session with OMQ and OMX
 *
 * Creates a tmux split-pane layout with Qoder CLI (OMQ) on the left
...
export interface InteropRuntimeFlags {
  enabled: boolean;
  mode: InteropMode;
  omqInteropToolsEnabled: boolean;
  failClosed: boolean;
}

export function readInteropRuntimeFlags(env: NodeJS.ProcessEnv = process.env): InteropRuntimeFlags {
  const rawMode = (env.OMX_OMQ_INTEROP_MODE || 'off').toLowerCase();
  const mode: InteropMode = rawMode === 'observe' || rawMode === 'active' ? rawMode : 'off';
  return {
    enabled: env.OMX_OMQ_INTEROP_ENABLED === '1',
    mode,
    omqInteropToolsEnabled: env.OMQ_INTEROP_TOOLS_ENABLED === '1',
    failClosed: env.OMX_OMQ_INTEROP_FAIL_CLOSED !== '0',
  };
}
...
export function validateInteropRuntimeFlags(flags: InteropRuntimeFlags): { ok: boolean; reason?: string } {
  if (!flags.enabled && flags.mode !== 'off') {
    return { ok: false, reason: 'OMX_OMQ_INTEROP_MODE must be "off" when OMX_OMQ_INTEROP_ENABLED=0.' };
  }

  if (flags.mode === 'active' && !flags.omqInteropToolsEnabled) {
    return { ok: false, reason: 'Active mode requires OMQ_INTEROP_TOOLS_ENABLED=1.' };
  }
...
export function launchInteropSession(cwd: string = process.cwd()): void {
  ...
  console.log(`[interop] mode=${flags.mode}, enabled=${flags.enabled ? '1' : '0'}, tools=${flags.omqInteropToolsEnabled ? '1' : '0'}, failClosed=${flags.failClosed ? '1' : '0'}`);
  if (!flagCheck.ok) {
  ...
  if (!hasCodex) {
    console.warn('Warning: codex CLI is not available. Only Qoder CLI will be launched.');
  ...
  try {
    if (hasCodex) {
      ...
      console.log('Splitting pane: Left (Qoder CLI) | Right (Codex)');
      ...
      console.log('- Left pane: Qoder CLI (this terminal)');
      ...
      console.log('\nQoder CLI is ready in this pane.');
