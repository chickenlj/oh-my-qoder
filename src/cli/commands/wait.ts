 ...
 * - Zero learning curve: `omq wait` just works
 * - Smart defaults: Auto-detects tmux and daemon status
 * - Minimal commands: Most users only need `omq wait`
 *
 * Commands:
 *   omq wait               - Smart command: shows status, offers to start daemon if needed
 *   omq wait status        - Show current rate limit and daemon status
...
export async function waitCommand(options: WaitOptions): Promise<void> {
  ...
  if (rateLimitStatus.isLimited) {
    ...
    if (!tmuxAvailable) {
      ...
      console.log(chalk.white('   Run: ') + chalk.green('omq wait --start'));
      console.log(chalk.gray('   (or: omq wait daemon start)\n'));
    } else {
    ...
    if (daemonRunning) {
      ...
      console.log(chalk.gray('Stop with: omq wait --stop\n'));
    }
  }
}
  ...
  if (action === 'start') {
    if (options.foreground) {
      ...
      if (result.success) {
        ...
        console.log(chalk.gray('  • Track blocked Qoder CLI sessions in tmux'));
        console.log(chalk.gray('  • Auto-resume sessions when rate limit clears'));
        console.log(chalk.gray('\nUse "omq wait status" to check daemon status'));
        console.log(chalk.gray('Use "omq wait daemon stop" to stop the daemon'));
      } else {
...
/**
 * Detect blocked Qoder CLI sessions
 */
export async function waitDetectCommand(options: WaitDetectOptions): Promise<void> {
  ...
  console.log(chalk.blue('Scanning for blocked Qoder CLI sessions...\n'));
  ...
  if (result.state?.blockedPanes && result.state.blockedPanes.length > 0) {
    ...
    console.log(chalk.gray('  omq wait daemon start'));
  }
