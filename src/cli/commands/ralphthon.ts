/**
 * omq ralphthon CLI subcommand
 *
 * Autonomous hackathon lifecycle:
 *   omq ralphthon "task"                  Start new ralphthon session
 *   omq ralphthon --resume                Resume existing session
 *   omq ralphthon --skip-interview "task" Skip deep-interview, use task directly
 *   omq ralphthon --max-waves 5           Set max hardening waves
 *   omq ralphthon --poll-interval 60      Set poll interval in seconds
 */
...
Usage: omq ralphthon [options] [task]
...
Examples:
  omq ralphthon "Build a REST API for user management"
  omq ralphthon --skip-interview "Implement auth middleware"
  omq ralphthon --resume
  omq ralphthon --max-waves 5 --poll-interval 60 "Add caching layer"
`;
...
After the interview, generate a ralphthon-prd.json file in .omq/ with this structure:
{
...
export async function ralphthonCommand(args: string[]): Promise<void> {
  ...
  if (!options.task) {
    console.error(
      chalk.red('Task description required. Usage: omq ralphthon "your task"'),
    );
    console.log(RALPHTHON_HELP);
    process.exit(1);
  }
  ...
  if (!isInsideTmux()) {
    console.error(
      chalk.red(
        "Ralphthon requires tmux. Run inside a tmux session or use `omq` to launch one.",
      ),
    );
    process.exit(1);
  }
