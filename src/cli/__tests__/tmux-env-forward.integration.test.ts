 ...
 *
 * Verifies that env vars set on the omq process actually arrive inside
...
describe.skipIf(!HAS_TMUX)('tmux env forwarding — integration', () => {
  const SESSION_NAME = `omq-env-test-${Date.now()}`;
  let tempDir: string;
  let outFile: string;

  beforeAll(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'omq-tmux-env-'));
  ...
  it('QODER_CONFIG_DIR set via buildEnvExportPrefix reaches the tmux pane', () => {
    const testValue = '/tmp/omq-test-config-dir';
