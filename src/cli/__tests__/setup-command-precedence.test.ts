/**
 * Real commander-pipeline tests for `omq setup --plugin-dir-mode` and the
 * OMQ_PLUGIN_ROOT auto-detection precedence.
 *
 ...
 *   2. OMQ_PLUGIN_ROOT env, no flag                 → opts.pluginDirMode === true + auto-detect log
 *   3. neither                                      → opts.pluginDirMode === false
 *   4. --plugin-dir-mode --no-plugin                → pluginDirMode=false, noPlugin=true, conflict warning
 *   5. OMQ_PLUGIN_ROOT + --no-plugin                → pluginDirMode=false, noPlugin=true, conflict warning
...
const ORIG_OMQ_PLUGIN_ROOT = process.env[OMQ_PLUGIN_ROOT_ENV];
...
afterEach(() => {
  if (ORIG_OMQ_PLUGIN_ROOT === undefined) {
    delete process.env[OMQ_PLUGIN_ROOT_ENV];
  } else {
    process.env[OMQ_PLUGIN_ROOT_ENV] = ORIG_OMQ_PLUGIN_ROOT;
  }
  logSpy.mockRestore();
  warnSpy.mockRestore();
...
describe('omq setup commander pipeline — pluginDirMode precedence', () => {
  ...
  it('2. OMQ_PLUGIN_ROOT env, no flag → pluginDirMode auto-enabled with detection log', async () => {
    process.env[OMQ_PLUGIN_ROOT_ENV] = '/tmp/foo';
    await runSetup([]);
    expect(lastInstallOptions().pluginDirMode).toBe(true);
    expect(loggedText()).toMatch(/Detected OMQ_PLUGIN_ROOT/);
  });
  ...
  it('5. OMQ_PLUGIN_ROOT env + --no-plugin → noPlugin wins, conflict warning logged', async () => {
