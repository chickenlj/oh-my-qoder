...
function writeAdvisorStub(dir: string): string {
  ...
  writeFileSync(
    ...
    [
      ...
      '  originalTask: process.env.OMQ_ASK_ORIGINAL_TASK ?? null,',
...
describe('parseAskArgs', () => {
  it('supports positional and print/prompt flag forms', () => {
    expect(parseAskArgs(['qwen', 'review', 'this'])).toEqual({ provider: 'qwen', prompt: 'review this' });
    expect(parseAskArgs(['gemini', '-p', 'brainstorm'])).toEqual({ provider: 'gemini', prompt: 'brainstorm' });
    expect(parseAskArgs(['qwen', '--print', 'draft', 'summary'])).toEqual({ provider: 'qwen', prompt: 'draft summary' });
  ...
  it('supports --agent-prompt flag and equals syntax', () => {
    expect(parseAskArgs(['qwen', '--agent-prompt', 'executor', 'do', 'it'])).toEqual({
      provider: 'qwen',
      prompt: 'do it',
...
describe('omq ask command', () => {
  it('accepts canonical advisor env and forwards prompt/task to advisor', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omq-ask-canonical-'));
    try {
      const stubPath = writeAdvisorStub(wd);
      const result = runCli(
        ['ask', 'qwen', '--print', 'hello world'],
        wd,
        { OMQ_ASK_ADVISOR_SCRIPT: stubPath },
      );
  ...
  it('accepts OMX advisor env alias in Phase-1 and emits deprecation warning', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omq-ask-alias-'));
    try {
      const stubPath = writeAdvisorStub(wd);
      const result = runCli(
  ...
  it('allows codex ask inside a Qoder CLI session', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omq-ask-cli-codex-nested-'));
    try {
      const stubPath = writeAdvisorStub(wd);
      const result = runCli(
  ...
  it('allows gemini ask inside a Qoder CLI session', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omq-ask-cli-gemini-nested-'));
    try {
      const stubPath = writeAdvisorStub(wd);
      const result = runCli(
  ...
  it('allows cursor ask inside a Qoder CLI session', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omq-ask-cli-cursor-nested-'));
    try {
      const stubPath = writeAdvisorStub(wd);
      const result = runCli(
  ...
  it('loads --agent-prompt role from resolved prompts dir and prepends role content', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omq-ask-agent-prompt-'));
    try {
      const stubPath = writeAdvisorStub(wd);
      ...
      const result = runCli(
        ['ask', 'qwen', '--agent-prompt=executor', '--prompt', 'ship feature'],
        wd,
        { OMQ_ASK_ADVISOR_SCRIPT: stubPath },
      );
...
describe.skip('run-provider-advisor script contract (scripts/run-provider-advisor.js not present)', () => {
  it('writes artifact to .omq/artifacts/ask/{provider}-{slug}-{timestamp}.md', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omq-ask-artifact-'));
    try {
      ...
      expect(artifactPath).toContain(join('.omq', 'artifacts', 'ask', 'claude-artifact-path-contract-'));
  ...
  it('accepts OMX original-task alias in Phase-1 with deprecation warning', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omq-ask-original-alias-'));
    try {
  ...
  it.each([
    ...
    const wd = mkdtempSync(join(tmpdir(), `omq-ask-${provider}-advisor-env-`));
    try {
  ...
  it('launches grok as `grok -p <prompt> --always-approve` and never pipes stdin', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omq-ask-grok-args-'));
    try {
      const capturePath = join(wd, 'spawn-sync-calls.json');
      const preludePath = writeSpawnSyncCapturePrelude(wd);
  ...
  it('launches cursor as `cursor-agent --print --force --trust --sandbox disabled <prompt>` and never pipes stdin', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omq-ask-cursor-args-'));
    try {
      const capturePath = join(wd, 'spawn-sync-calls.json');
      const preludePath = writeSpawnSyncCapturePrelude(wd);
  ...
  it('sanitizes Rust env vars for codex so artifacts do not capture Rust stderr logs', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omq-ask-codex-rust-env-'));
    try {
  ...
  it('pipes the Windows codex prompt over stdin to avoid shell arg splitting', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omq-ask-codex-win32-shell-'));
    try {
      const capturePath = join(wd, 'spawn-sync-calls.json');
      const preludePath = writeSpawnSyncCapturePrelude(wd);
      const result = runAdvisorScriptWithPrelude(
  ...
  it('pipes the Windows gemini prompt over stdin to avoid --prompt conflicts and AttachConsole failures', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omq-ask-gemini-win32-stdin-'));
    try {
      const capturePath = join(wd, 'spawn-sync-calls.json');
      const preludePath = writeSpawnSyncCapturePrelude(wd);
      const result = runAdvisorScriptWithPrelude(
  ...
  it('pipes multiline codex prompts over stdin on non-Windows shells', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omq-ask-codex-multiline-stdin-'));
  ...
  it('pipes long gemini prompts over stdin on non-Windows shells', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omq-ask-gemini-long-stdin-'));
  ...
  it('pipes multiline claude prompts over stdin so the prompt is never a raw argv value (#3221)', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omq-ask-claude-multiline-stdin-'));
    const multilinePrompt = 'line one\nline two\nline three';
    try {
      const capturePath = join(wd, 'spawn-sync-calls.json');
  ...
  it('pipes frontmatter claude prompts over stdin so a leading dash is not parsed as a CLI option (#3221)', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omq-ask-claude-frontmatter-stdin-'));
  ...
  it('pipes a short claude prompt that begins with a dash over stdin instead of as argv (#3221)', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omq-ask-claude-leading-dash-stdin-'));
  ...
  it('keeps a short single-line claude prompt as a `-p <prompt>` argv without piping stdin', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omq-ask-claude-short-argv-'));
  ...
  it('shows install guidance when a Windows codex binary is missing under shell:true', () => {
    const wd = mkdtempSync(join(tmpdir(), 'omq-ask-codex-win32-missing-'));
    try {
      const capturePath = join(wd, 'spawn-sync-calls.json');
      const preludePath = writeSpawnSyncCapturePrelude(wd);
      const result = runAdvisorScriptWithPrelude(
  ...
  it.each([
    ...
    const wd = mkdtempSync(join(tmpdir(), `omq-ask-${provider}-stdin-close-`));
    try {
      const capturePath = join(wd, 'spawn-sync-calls.json');
      const preludePath = writeSpawnSyncCapturePreludeNative(wd);
      const result = runAdvisorScriptWithPrelude(
