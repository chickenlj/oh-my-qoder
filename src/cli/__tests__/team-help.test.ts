...
describe('team cli help text surfaces', () => {
  it('team.ts usage includes legacy and api surfaces', () => {
    ...
    expect(source).toContain('omq team resume <team_name>');
    expect(source).toContain('omq team shutdown <team_name>');
    expect(source).toContain('omq team api <operation>');
    expect(source).toContain('omq team [ralph] <N:agent-type[:role]>');
  });
