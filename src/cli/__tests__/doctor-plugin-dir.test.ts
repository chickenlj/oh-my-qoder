/**
 * Tests for --plugin-dir support in `omq doctor` and `omq doctor conflicts`.
 *
 ...
 *
 * OMQ_CLI_SKIP_PARSE prevents index.ts from calling program.parse() at import
 * time (which would trigger launchCommand → process.exit inside the test
 * worker that inherits CLAUDECODE from the parent Qoder CLI session).
 */
...
describe('applyPluginDirOption', () => {
  ...
  beforeEach(() => {
    savedEnv = process.env[OMQ_PLUGIN_ROOT_ENV];
  ...
  afterEach(() => {
    if (savedEnv === undefined) {
      ...
      process.env[OMQ_PLUGIN_ROOT_ENV] = savedEnv;
    }
    warnSpy.mockRestore();
  });
  ...
  it('resolves a relative path to absolute', async () => {
    ...
    expect(process.env[OMQ_PLUGIN_ROOT_ENV]).toBe(resolve('./rel/path'));
  });

  it('is a no-op when rawPath is undefined', async () => {
    const { applyPluginDirOption } = await import('../index.js');
    applyPluginDirOption(undefined);
    expect(process.env[OMQ_PLUGIN_ROOT_ENV]).toBeUndefined();
  });

  it('wins over a pre-set OMQ_PLUGIN_ROOT env var', async () => {
  ...
  it('relative path (subcommand form) is resolved to absolute', async () => {
    const { applyPluginDirOption } = await import('../index.js');
    // Simulates: omq doctor conflicts --plugin-dir ./mydir
    applyPluginDirOption('./mydir');
    expect(process.env[OMQ_PLUGIN_ROOT_ENV]).toBe(resolve('./mydir'));
  });
});
...
describe('Commander integration: doctor --plugin-dir wiring', () => {
  ...
  it('parseAsync doctor conflicts --plugin-dir /tmp/foo sets OMQ_PLUGIN_ROOT', async () => {
    ...
    const savedEnv = process.env[OMQ_PLUGIN_ROOT_ENV];
    delete process.env[OMQ_PLUGIN_ROOT_ENV];
