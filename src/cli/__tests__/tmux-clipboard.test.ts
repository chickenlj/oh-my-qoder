...
describe('tmux clipboard configuration', () => {
  ...
  it('sets session-scoped clipboard options and appends universal terminal clipboard when missing', () => {
    ...
    configureTmuxClipboardForSession('omq-session', { stripTmux: true, stdio: 'ignore' });

    expect(mocks.tmuxExec).toHaveBeenCalledWith(
      ['set-option', '-t', 'omq-session', 'set-clipboard', 'on'],
      { stripTmux: true, stdio: 'ignore' },
    );
    expect(mocks.tmuxExec).toHaveBeenCalledWith(
      ['show-options', '-t', 'omq-session', '-v', 'terminal-features'],
      { stripTmux: true, stdio: 'ignore' },
    );
    expect(mocks.tmuxExec).toHaveBeenCalledWith(
      ['set-option', '-at', 'omq-session', 'terminal-features', ',*:clipboard'],
      { stripTmux: true, stdio: 'ignore' },
    );
  });
  ...
  it('does not append terminal-features when universal clipboard is already present', () => {
    ...
    configureTmuxClipboardForSession('omq-session');
  ...
  it('supports async tmux launch paths', async () => {
    ...
    await configureTmuxClipboardForSessionAsync('omq-team');

    expect(mocks.tmuxExecAsync).toHaveBeenCalledWith(['set-option', '-t', 'omq-team', 'set-clipboard', 'on'], undefined);
    expect(mocks.tmuxExecAsync).toHaveBeenCalledWith(['set-option', '-at', 'omq-team', 'terminal-features', ',*:clipboard'], undefined);
  });
});
