/**
 * Tests for `--plugin-dir` capture in `omq` launch.
 *
 * Plan: binary-weaving-mountain — HUD wrapper resolves the active plugin root
 * from `process.env.OMQ_PLUGIN_ROOT`, set by the `omq` CLI when the user
 * passes `--plugin-dir <path>`. The flag must NOT be consumed (it still
 * forwards to Qoder CLI's plugin loader untouched).
 */
...
describe('parsePluginDirArg', () => {
  ...
  it('preserves Windows drive-letter absolute paths on non-Windows hosts', () => {
    const out = parsePluginDirArg(['--plugin-dir', 'C:\\Users\\me\\omq']);
    expect(out).toBe('C:\\Users\\me\\omq');
  });

  it('preserves Windows UNC absolute paths on non-Windows hosts', () => {
    const out = parsePluginDirArg(['--plugin-dir=\\\\server\\share\\omq']);
    expect(out).toBe('\\\\server\\share\\omq');
  });
...
describe('OMQ_PLUGIN_ROOT tmux env forwarding', () => {
  it('is included in TMUX_ENV_FORWARD so it survives tmux env scrubbing', () => {
    expect(TMUX_ENV_FORWARD).toContain('OMQ_PLUGIN_ROOT');
  });
});
 ...
 *
 * The thing under test: `launchCommand` mutates `process.env[OMQ_PLUGIN_ROOT_ENV]`
...
describe('launchCommand → child env propagation (OMQ_PLUGIN_ROOT)', () => {
  let tmpConfigDir: string;
  let savedEnv: { [k: string]: string | undefined };
  let savedCwd: string;

  beforeEach(() => {
    tmpConfigDir = mkdtempSync(join(tmpdir(), 'omq-pdc-'));
    savedEnv = {
      [OMQ_PLUGIN_ROOT_ENV]: process.env[OMQ_PLUGIN_ROOT_ENV],
      QODER_CONFIG_DIR: process.env.QODER_CONFIG_DIR,
      CLAUDECODE: process.env.CLAUDECODE,
      OMQ_NOTIFY: process.env.OMQ_NOTIFY,
    };
  ...
  it('3. no flag and no parent env → child env does not contain OMQ_PLUGIN_ROOT', async () => {
    await runLaunch([]);
    expect(capturedEnv).not.toBeNull();
    expect(capturedEnv![OMQ_PLUGIN_ROOT_ENV]).toBeUndefined();
  });
  ...
  it('5. parent env set + no flag → child inherits parent OMQ_PLUGIN_ROOT', async () => {
    process.env[OMQ_PLUGIN_ROOT_ENV] = '/tmp/bar';
    await runLaunch([]);
    expect(capturedEnv![OMQ_PLUGIN_ROOT_ENV]).toBe('/tmp/bar');
  });
  ...
  it('6. relative --plugin-dir is resolved against the launch CWD', async () => {
    ...
    const knownCwd = realpathSync(mkdtempSync(join(tmpdir(), 'omq-pdc-cwd-')));
    try {
      process.chdir(knownCwd);
      await runLaunch(['--plugin-dir', './foo']);
      expect(capturedEnv![OMQ_PLUGIN_ROOT_ENV]).toBe(resolve(knownCwd, './foo'));
    } finally {
      try { process.chdir(savedCwd); } catch { /* ignore */ }
