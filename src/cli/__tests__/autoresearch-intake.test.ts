...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'omq-autoresearch-intake-test-'));
...
describe('autoresearch intake draft artifacts', () => {
  it('writes a canonical deep-interview autoresearch draft artifact from vague input', async () => {
    ...
    try {
      ...
      expect(artifact.path).toMatch(/\.omq\/specs\/deep-interview-autoresearch-improve-onboarding-for-first-time-contributors\.md$/);
  ...
  it('writes launch-consumable mission/sandbox/result artifacts', async () => {
    ...
    try {
      ...
      expect(resultJson.kind).toBe('omq.autoresearch.deep-interview/v1');
