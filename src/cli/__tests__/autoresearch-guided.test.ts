...
describe('spawnAutoresearchTmux', () => {
  ...
  it('uses explicit cwd, login-shell wrapping, and verifies startup before logging success', () => {
    ...
    expect(logSpy).toHaveBeenCalledWith('  Attach:   tmux attach -t omq-autoresearch-demo');
  });
});
...
describe('spawnAutoresearchSetupTmux', () => {
  ...
  it('launches a detached claude setup session and seeds deep-interview autoresearch mode', async () => {
    ...
    try {
      ...
      tmuxExecMock.mockImplementation((args: string[]) => {
        if (args[0] === 'new-session') {
          expect(args.slice(0, 9)).toEqual([
            'new-session', '-d', '-P', '-F', '#{pane_id}', '-s', 'omq-autoresearch-setup-kf12oi', '-c', repo,
          ]);
        ...
        if (args[0] === 'has-session') {
          hasSessionCalls += 1;
          expect(args).toEqual(['has-session', '-t', 'omq-autoresearch-setup-kf12oi']);
          return '';
        }
        if (args[0] === 'send-keys') {
          return '';
      ...
      expect(tmuxExecMock).toHaveBeenCalledWith(['set-option', '-t', 'omq-autoresearch-setup-kf12oi', 'set-clipboard', 'on'], { stripTmux: true, stdio: 'ignore' });
      expect(tmuxExecMock).toHaveBeenCalledWith(['set-option', '-at', 'omq-autoresearch-setup-kf12oi', 'terminal-features', ',*:clipboard'], { stripTmux: true, stdio: 'ignore' });
      expect(logSpy).toHaveBeenCalledWith('  Attach:   tmux attach -t omq-autoresearch-setup-kf12oi');
