...
import type { OmqTeamJob } from './team-job-convergence.js';
import { getGlobalOmqStatePath } from '../utils/paths.js';

const omqTeamJobs = new Map<string, OmqTeamJob>();
const OMQ_JOBS_DIR = process.env.OMQ_JOBS_DIR || getGlobalOmqStatePath('team-jobs');
...
type DeprecatedTeamToolName =
  ...
  | 'omq_run_team_status'
  | 'omq_run_team_wait'
  | 'omq_run_team_cleanup';

const TEAM_CLI_REPLACEMENT_HINTS: Record<DeprecatedTeamToolName, string> = {
  omq_run_team_start: 'omq team start',
  omq_run_team_status: 'omq team status <job_id>',
  omq_run_team_wait: 'omq team wait <job_id>',
  omq_run_team_cleanup: 'omq team cleanup <job_id>',
};
...
function buildCliReplacement(toolName: DeprecatedTeamToolName, args: unknown): string {
  ...
  if (toolName === 'omq_run_team_start') {
    ...
    const flags: string[] = ['omq', 'team', 'start'];
  ...
  if (toolName === 'omq_run_team_status') {
    return `omq team status --job-id ${quoteCliValue(jobId)}`;
  }

  if (toolName === 'omq_run_team_wait') {
    const timeoutMs = typeof parsed.timeout_ms === 'number' && Number.isFinite(parsed.timeout_ms)
      ? ` --timeout-ms ${Math.floor(parsed.timeout_ms)}`
      : '';
    return `omq team wait --job-id ${quoteCliValue(jobId)}${timeoutMs}`;
  }

  if (toolName === 'omq_run_team_cleanup') {
    const graceMs = typeof parsed.grace_ms === 'number' && Number.isFinite(parsed.grace_ms)
      ? ` --grace-ms ${Math.floor(parsed.grace_ms)}`
      : '';
    return `omq team cleanup --job-id ${quoteCliValue(jobId)}${graceMs}`;
  }
  ...
  return {
    content: [{
      ...
      text: JSON.stringify({
        ...
        tool: toolName,
        message: 'Legacy team MCP runtime tools are deprecated. Use the omq team CLI instead.',
...
function persistJob(jobId: string, job: OmqTeamJob): void {
  try {
    if (!existsSync(OMQ_JOBS_DIR)) mkdirSync(OMQ_JOBS_DIR, { recursive: true });
    writeFileSync(join(OMQ_JOBS_DIR, `${jobId}.json`), JSON.stringify(job), 'utf-8');
  } catch { /* best-effort */ }
}

function loadJobFromDisk(jobId: string): OmqTeamJob | undefined {
  try {
    return JSON.parse(readFileSync(join(OMQ_JOBS_DIR, `${jobId}.json`), 'utf-8')) as OmqTeamJob;
  } catch {
    return undefined;
  }
}
...
async function loadPaneIds(jobId: string): Promise<{ paneIds: string[]; leaderPaneId: string; sessionName?: string; ownsWindow?: boolean } | null> {
  const p = join(OMQ_JOBS_DIR, `${jobId}-panes.json`);
  try { return JSON.parse(await readFile(p, 'utf-8')); }
  catch { return null; }
}


async function resolveCleanupPaneEvidence(job: OmqTeamJob, jobId: string): Promise<{
...
function validateJobId(job_id: string): void {
  if (!/^omq-[a-z0-9]{1,16}$/.test(job_id)) {
    throw new Error(`Invalid job_id: "${job_id}". Must match /^omq-[a-z0-9]{1,16}$/`);
  }
}

function saveJobState(jobId: string, job: OmqTeamJob): OmqTeamJob {
  omqTeamJobs.set(jobId, job);
  persistJob(jobId, job);
  return job;
}

function makeJobResponse(jobId: string, job: OmqTeamJob, extra: Record<string, unknown> = {}): { content: Array<{ type: 'text'; text: string }> } {
...
const startSchema = z.object({
  ...
  agentTypes: z.array(z.string()).describe('Agent type per worker: "qwen", "codex", or "gemini"'),
...
const statusSchema = z.object({
  job_id: z.string().describe('Job ID returned by omq_run_team_start'),
});
...
async function handleStart(args: unknown): Promise<{ content: Array<{ type: 'text'; text: string }> }> {
  if (
    ...
    throw new Error(
      'omq_run_team_start no longer accepts timeoutSeconds. Remove timeoutSeconds and use omq_run_team_wait timeout_ms to limit the wait call only (workers keep running until completion or explicit omq_run_team_cleanup).',
    );
  }

  const input = startSchema.parse(args);
  validateTeamName(input.teamName);
  const jobId = `omq-${Date.now().toString(36)}${randomUUID().slice(0, 8)}`;
  const runtimeCliPath = join(__ownDir, 'runtime-cli.cjs');

  const job: OmqTeamJob = { status: 'running', startedAt: Date.now(), teamName: input.teamName, cwd: input.cwd };
  omqTeamJobs.set(jobId, job);

  const child = spawn(process.execPath, [runtimeCliPath], {
    env: { ...process.env, OMQ_JOB_ID: jobId, OMQ_JOBS_DIR },
  ...
  return {
    content: [{ type: 'text', text: JSON.stringify({ jobId, pid: job.pid, message: 'Team started. Poll with omq_run_team_status.' }) }],
  };
}
...
export async function handleStatus(args: unknown): Promise<{ content: Array<{ type: 'text'; text: string }> }> {
  ...
  let job = omqTeamJobs.get(job_id) ?? loadJobFromDisk(job_id);
  if (!job) {
  ...
  const artifactConvergence = convergeJobWithResultArtifact(job, job_id, OMQ_JOBS_DIR);
...
export async function handleWait(args: unknown): Promise<{ content: Array<{ type: 'text'; text: string }> }> {
  ...
  const startedAt = omqTeamJobs.get(job_id)?.startedAt ?? Date.now();
  const elapsed = ((Date.now() - startedAt) / 1000).toFixed(1);
  const timeoutOut: Record<string, unknown> = {
    error: `Timed out waiting for job ${job_id} after ${(timeout_ms / 1000).toFixed(0)}s — workers are still running; call omq_run_team_wait again to keep waiting or omq_run_team_cleanup to stop them`,
    jobId: job_id,
    status: 'running',
...
export async function handleCleanup(args: unknown): Promise<{ content: Array<{ type: 'text'; text: string }> }> {
  ...
  const job = omqTeamJobs.get(job_id) ?? loadJobFromDisk(job_id);
...
const TOOLS = [
  {
    ...
    description: '[DEPRECATED] CLI-only migration required. This tool no longer executes; use `omq team start`.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        teamName: { type: 'string', description: 'Slug name for the team' },
        agentTypes: { type: 'array', items: { type: 'string' }, description: '"qwen", "codex", or "gemini" per worker' },
        tasks: {
          type: 'array',
          items: {
            type: 'object',
  ...
  },
  {
    ...
    description: '[DEPRECATED] CLI-only migration required. This tool no longer executes; use `omq team status <job_id>`.',
    inputSchema: {
      type: 'object' as const,
      properties: {
  ...
  },
  {
    ...
    description: '[DEPRECATED] CLI-only migration required. This tool no longer executes; use `omq team wait <job_id>`.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        job_id: { type: 'string', description: 'Job ID returned by omq_run_team_start' },
  ...
  },
  {
    ...
    description: '[DEPRECATED COMPAT] Prefer `omq team cleanup <job_id>`; this compatibility cleanup surface preserves team state when worker liveness or worktree cleanup is not proven safe.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        job_id: { type: 'string', description: 'Job ID returned by omq_run_team_start' },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    if (name === 'omq_run_team_start') return await handleStart(args ?? {});
    if (name === 'omq_run_team_status') return await handleStatus(args ?? {});
    if (name === 'omq_run_team_wait') return await handleWait(args ?? {});
    if (name === 'omq_run_team_cleanup') return await handleCleanup(args ?? {});
  } catch (error) {
...
async function main() {
  ...
  console.error('OMQ Team MCP Server running on stdio');
}

if (process.env.OMQ_TEAM_SERVER_DISABLE_AUTOSTART !== '1' && process.env.NODE_ENV !== 'test') {
  main().catch((error) => {
