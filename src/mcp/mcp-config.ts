 ...
 * - OMQ_MCP_OUTPUT_PATH_POLICY=strict|redirect_output (default: strict)
 * - OMQ_MCP_OUTPUT_REDIRECT_DIR=.omq/outputs (default: .omq/outputs)
 * - OMQ_MCP_ALLOW_EXTERNAL_PROMPT=0|1 (default: 0)
 *
...
export function loadMcpConfig(): McpConfig {
  const outputPathPolicy = parseOutputPathPolicy(process.env.OMQ_MCP_OUTPUT_PATH_POLICY);
  const outputRedirectDir = process.env.OMQ_MCP_OUTPUT_REDIRECT_DIR || DEFAULT_MCP_CONFIG.outputRedirectDir;
  const allowExternalPrompt = parseBooleanEnv(process.env.OMQ_MCP_ALLOW_EXTERNAL_PROMPT, DEFAULT_MCP_CONFIG.allowExternalPrompt);
  ...
  if (config.allowExternalPrompt) {
    console.warn('[MCP Config] WARNING: OMQ_MCP_ALLOW_EXTERNAL_PROMPT is enabled. External prompt files outside the working directory are allowed. This may pose a security risk.');
  }
