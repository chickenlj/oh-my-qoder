#!/usr/bin/env node
/**
 * Standalone MCP Server for OMQ Tools
 *
 * This server exposes LSP, AST, and Python REPL tools via stdio transport
 * for discovery by Qoder CLI's MCP management system.
 *
...
async function gracefulShutdown(signal: string): Promise<void> {
  ...
  console.error(`OMQ MCP Server: received ${signal}, disconnecting LSP servers...`);
...
async function main() {
  ...
  console.error('OMQ Tools MCP Server running on stdio');
}
