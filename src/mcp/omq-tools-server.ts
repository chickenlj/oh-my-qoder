/**
 * OMQ Tools Server - In-process MCP server for custom tools
 *
 ...
 */
export const omqToolNames = enabledTools.map(t => `mcp__t__${t.name}`);

// Build a map from MCP tool name to category for efficient lookup
// Built from allTools so getOmqToolNames() category filtering works correctly
 ...
 */
export function getOmqToolNames(options?: ToolNameFilterOptions): string[] {
  return filterToolNames(omqToolNames, toolCategoryMap, options);
}
