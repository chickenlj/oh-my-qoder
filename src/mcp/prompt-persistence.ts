 ...
 *
 * Writes assembled prompts and model responses to .omq/prompts/ before/after
...
import { getWorktreeRoot, getOmqRoot } from '../lib/worktree-paths.js';
import {
...
const PROMPT_PERSISTENCE_PRODUCER = { system: 'omq', component: 'prompt-persistence' } as const;
...
export function getPromptsDir(workingDirectory?: string): string {
  const root = getWorktreeRoot(workingDirectory) || workingDirectory || process.cwd();
  return join(getOmqRoot(root), 'prompts');
}
