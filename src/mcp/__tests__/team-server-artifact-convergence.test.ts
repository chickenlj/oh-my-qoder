...
describe('team-server artifact convergence + scoped cleanup', () => {
  ...
  beforeEach(() => {
    testRoot = join(tmpdir(), `omq-team-server-test-${process.pid}-${Date.now()}`);
