...
export async function render(
  ...
  if (enabledElements.omqLabel) {
    const localSuffix = isRuntimePackageLocal() ? "L" : "";
    const versionTag = context.omqVersion
      ? `#${context.omqVersion}${localSuffix}`
      : (localSuffix ? `#${localSuffix}` : "");
    if (enabledElements.updateNotification !== false && context.updateAvailable) {
      rendered.set(
        "omqLabel",
        bold(`[OMQ${versionTag}] -> ${context.updateAvailable} omq update`),
      );
    } else {
      rendered.set("omqLabel", bold(`[OMQ${versionTag}]`));
    }
  }
