 ...
 *
 * Authentication:
 * - macOS: Reads from Keychain "Qoder CLI-credentials"
...
/**
 * OAuth client_id for Qoder CLI (public client).
 ...
 * Qoder CLI uses "Qoder CLI-credentials-{sha256(configDir)[:8]}" for
 ...
 * that behavior so ~-prefixed profiles keep matching Qoder CLI's own
 * Keychain entries.
 */
function getKeychainServiceName(): string {
  ...
  if (configDir) {
    ...
    return `Qoder CLI-credentials-${hash}`;
  }
  return 'Qoder CLI-credentials';
}
...
function fetchUsageFromZai(): Promise<FetchResult<ZaiQuotaResponse>> {
  return new Promise((resolve) => {
    ...
    const authToken = process.env.DASHSCOPE_AUTH_TOKEN;
...
export async function getUsage(): Promise<UsageResult> {
  ...
  try {
    return await withFileLock(lockPathFor(getCachePath(currentSource)), async () => {
      ...
      // Anthropic OAuth path (official Qoder CLI support)
