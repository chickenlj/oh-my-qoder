/**
 * OMQ HUD - API Key Source Element
 *
 * Detects and renders where the active DASHSCOPE_API_KEY comes from:
...
/**
 * Check whether a settings file defines DASHSCOPE_API_KEY in its env block.
 */
function settingsFileHasApiKey(filePath: string): boolean {
  try {
    ...
    return 'DASHSCOPE_API_KEY' in env;
  } catch {
    return false;
  }
}
...
/**
 * Detect where the active DASHSCOPE_API_KEY comes from.
 *
 * Priority:
...
export function detectApiKeySource(cwd?: string): ApiKeySource | null {
  ...
  // 3. Environment variable
  if (process.env.DASHSCOPE_API_KEY) return 'env';
