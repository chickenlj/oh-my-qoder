...
function getAgentCode(agentType: string, model?: string): string {
  // Extract the short name from full type (e.g., "oh-my-qoder:architect" -> "architect")
  ...
  if (model) {
    ...
    if (code.length === 1) {
      code = tier.includes('high') ? code.toUpperCase() : code.toLowerCase();
    } else {
      const first = tier.includes('high') ? code[0].toUpperCase() : code[0].toLowerCase();
...
export function renderAgentsDetailed(agents: ActiveAgent[]): string | null {
  ...
  const names = running.map((a) => {
    // Extract last part of agent type (e.g., "oh-my-qoder:explore" -> "explore")
