 ...
 * - High (qwen-max): Magenta (high-powered)
 * - Medium (qwen-plus): Yellow (standard)
 * - Low (qwen-turbo): Green (lightweight)
 */
export function getModelTierColor(model: string | undefined): string {
  ...
  if (tier.includes('high') || tier.includes('max')) return MAGENTA;
  if (tier.includes('medium') || tier.includes('plus')) return YELLOW;
  if (tier.includes('low') || tier.includes('turbo')) return GREEN;
