 ...
 * Helps users verify HUD setup after omq-setup.
 */
function showDiagnostic(): void {
  ...
  try {
    ...
    if (sl && typeof sl === "object" && typeof (sl as Record<string, unknown>).command === "string") {
      statusLineOk = ((sl as Record<string, unknown>).command as string).includes("omq-hud");
    } else if (typeof sl === "string") {
      statusLineOk = sl.includes("omq-hud");
    }
  } catch {
  ...
  console.log(`[OMQ] HUD v${version} | preset: ${preset}`);
...
async function main(watchMode = false, skipInit = false): Promise<void> {
  try {
    ...
    if (sessionSummaryEnabled && resolvedTranscriptPath && currentSessionId) {
      const omqStateDir = join(getOmqRoot(cwd), "state");
      sessionSummary = readSessionSummary(omqStateDir, currentSessionId);
    ...
    // Debug: log data if OMQ_DEBUG is set
    if (process.env.OMQ_DEBUG) {
      console.error(
    ...
    if (
      ...
      try {
        ...
        const triggerFile = join(omqStateDir, "compact-requested.json");
        writeFileSync(
          triggerFile,
          JSON.stringify({
    ...
    if (isInstallError) {
      console.log("[OMQ] run /omq-setup to install properly");
    } else {
      // Output fallback message to stdout for status line visibility
      console.log("[OMQ] HUD error - check stderr");
...
// Export for programmatic use (e.g., omq hud --watch loop)
export { main };

// Auto-run (unconditional so dynamic import() via omq-hud.mjs wrapper works correctly)
main();
