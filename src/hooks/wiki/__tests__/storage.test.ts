...
describe('Wiki Storage', () => {
  ...
  describe('getWikiDir', () => {
    it('should return .omq/wiki path', () => {
      const dir = getWikiDir(tempDir);
      expect(dir).toBe(path.join(tempDir, '.omq', 'wiki'));
    });
  });
  ...
  describe('ensureWikiDir', () => {
    ...
    it('should create .gitignore with wiki/ entry', () => {
      ...
      const gitignorePath = path.join(tempDir, '.omq', '.gitignore');
    ...
    it('should append to existing .gitignore', () => {
      ...
      fs.mkdirSync(omqDir, { recursive: true });
      fs.writeFileSync(path.join(omqDir, '.gitignore'), 'state/\n');

      ensureWikiDir(tempDir);
      const content = fs.readFileSync(path.join(omqDir, '.gitignore'), 'utf-8');
    ...
    it('should not duplicate wiki/ in .gitignore', () => {
      const omqDir = path.join(tempDir, '.omq');
      fs.mkdirSync(omqDir, { recursive: true });
      fs.writeFileSync(path.join(omqDir, '.gitignore'), 'wiki/\n');
