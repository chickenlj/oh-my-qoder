...
function loadWikiConfig(root: string): WikiConfig {
  try {
    const configPath = join(getOmqRoot(root), '.omq-config.json');
    // Try active Claude config too
    const activeConfigPath = join(getQoderConfigDir(), '.omq-config.json');
...
function feedProjectMemory(root: string): void {
  try {
    const pmPath = join(getOmqRoot(root), 'project-memory.json');
