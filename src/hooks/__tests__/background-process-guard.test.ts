...
describe('Background Process Guard (issue #302)', () => {
  ...
  beforeEach(() => {
    claudeConfigDir = mkdtempSync(join(tmpdir(), 'omq-bg-perms-'));
    process.env = { ...originalEnv, QODER_CONFIG_DIR: claudeConfigDir };
  ...
  describe('Bash tool with run_in_background=true', () => {
    ...
    it('should block repo-scoped inspection Bash commands from non-git temp dirs', async () => {
      const nonGitDir = mkdtempSync(join(tmpdir(), 'omq-bg-readonly-non-git-'));
    ...
    it('should block single-test Bash commands from non-git temp dirs', async () => {
      const nonGitDir = mkdtempSync(join(tmpdir(), 'omq-bg-testcmd-non-git-'));
