...
describe("_openclaw.wake", () => {
  ...
  it("is a no-op when OMQ_OPENCLAW is not set", () => {
    vi.stubEnv("OMQ_OPENCLAW", "");
    // Should return undefined without doing anything
    const result = _openclaw.wake("session-start", { sessionId: "sid-1" });
    expect(result).toBeUndefined();
  });

  it("is a no-op when OMQ_OPENCLAW is not '1'", () => {
    vi.stubEnv("OMQ_OPENCLAW", "true");
    const result = _openclaw.wake("session-start", { sessionId: "sid-1" });
    expect(result).toBeUndefined();
  });

  it("triggers the dynamic import when OMQ_OPENCLAW === '1'", async () => {
  ...
  it("logs when wakeOpenClaw rejects but does not throw", async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      '[omq] hooks.bridge openclaw wake failed for session-start: gateway down',
    );

    vi.doUnmock("../../openclaw/index.js");
  });

  it("does not throw when OMQ_OPENCLAW === '1' and import fails", async () => {
    vi.stubEnv("OMQ_OPENCLAW", "1");
  ...
  it("accepts all supported hook event types", () => {
    vi.stubEnv("OMQ_OPENCLAW", "");
    // These should all be callable without type errors (no-op since OMQ_OPENCLAW not set)
...
describe("bridge-level regression tests", () => {
  ...
  it("pre-tool-use emits only the dedicated ask-user-question OpenClaw signal", async () => {
    ...
    process.env.OMQ_NOTIFY = "0"; // suppress real notifications
