...
export interface HookOutput {
  continue: boolean;
  /** System message for context injection (Qoder CLI compatible) */
...
export function getCheckpointPath(directory: string): string {
  const checkpointDir = join(getOmqRoot(directory), "state", CHECKPOINT_DIR);
...
export async function exportWisdomToNotepad(
  ...
  const notepadsDir = join(getOmqRoot(directory), "notepads");
...
function readTodoSummary(directory: string): {
  ...
  const todoPaths = [
    ...
    join(getOmqRoot(directory), "state", "todos.json"),
  ];
