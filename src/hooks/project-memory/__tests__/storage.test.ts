...
describe('Project Memory Storage', () => {
  ...
  describe('getMemoryPath', () => {
    it('should return correct memory file path', () => {
      ...
      expect(memoryPath).toBe(path.join(projectRoot, '.omq', 'project-memory.json'));
    });

    it('should return centralized memory file path when OMQ_STATE_DIR is set', async () => {
  ...
  describe('saveProjectMemory', () => {
    it('should create .omq directory and save memory file', async () => {
      ...
      // Verify .omq directory exists
      const omqDir = path.join(projectRoot, '.omq');
    ...
    it('should save to centralized state dir without creating local .omq when OMQ_STATE_DIR is set', async () => {
      const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), 'project-memory-state-'));
      try {
        process.env.OMQ_STATE_DIR = stateDir;
        ...
        await expect(fs.access(path.join(projectRoot, '.omq', 'project-memory.json'))).rejects.toThrow();
      } finally {
        delete process.env.OMQ_STATE_DIR;
        await fs.rm(stateDir, { recursive: true, force: true });
      }
  ...
  describe('loadProjectMemory', () => {
    ...
    it('should return null for invalid JSON', async () => {
      // Create .omq directory
      const omqDir = path.join(projectRoot, '.omq');
      await fs.mkdir(omqDir, { recursive: true });
