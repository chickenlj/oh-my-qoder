...
/**
 * Load auto-invocation config from ~/.qoder/.omq-config.json
 */
export function loadInvocationConfig(): InvocationConfig {
  const configPath = path.join(getQoderConfigDir(), '.omq-config.json');
...
export function saveInvocationHistory(state: AutoInvokeState): void {
  const historyDir = path.join(os.homedir(), '.omq', 'analytics', 'invocations');
