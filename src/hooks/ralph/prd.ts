...
import { ensureSessionStateDir, getOmqRoot, getSessionStateDir } from '../../lib/worktree-paths.js';
 ...
 */
export function getOmqPrdPath(directory: string): string {
  return join(getOmqRoot(directory), PRD_FILENAME);
}
...
export function getLegacyStatePrdPath(directory: string): string {
  return join(getOmqRoot(directory), 'state', PRD_FILENAME);
}
...
export function findPrdPath(directory: string, sessionId?: string): string | null {
  ...
  const omqPath = getOmqPrdPath(directory);
...
export function writePrd(directory: string, prd: PRD, sessionId?: string): boolean {
  ...
  if (sessionId) {
    ...
    prdPath = findPrdPath(directory) ?? getOmqPrdPath(directory);
  }
