 ...
 * Qoder CLI UI bug #17088 (false "hook error" labels on MSYS2/Git Bash)
 * is avoided.
 */
...
describe('patchHooksJsonForWindows', () => {
  ...
  beforeEach(() => {
    pluginRoot = mkdtempSync(join(tmpdir(), 'omq-win-patch-'));
            ...
            hooks: [
              {
                type: 'command',
                command:
                  'sh "${QODER_PLUGIN_ROOT}/scripts/find-node.sh" "${QODER_PLUGIN_ROOT}/scripts/session-start.mjs"',
              },
            ],
          },
        ],
