 ...
 * when OMQ upgrades to a new version. Uses safe replace strategy:
...
describe('ensureStdinSymlink', () => {
  ...
  beforeEach(() => {
    ...
    pluginRoot = mkdtempSync(join(tmpdir(), 'omq-stdin-'));
    ...
    // Create a fake config directory and set QODER_CONFIG_DIR env var
    configDir = mkdtempSync(join(tmpdir(), 'omq-config-'));
  ...
  it('uses QODER_CONFIG_DIR when set', () => {
    // Set a custom config dir
    const customConfigDir = mkdtempSync(join(tmpdir(), 'omq-custom-config-'));
    const customHooksLib = join(customConfigDir, 'hooks/lib');
    process.env.QODER_CONFIG_DIR = customConfigDir;
