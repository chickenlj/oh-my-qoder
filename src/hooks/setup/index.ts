 ...
 * Setup Hook Module
 *
 * Handles OMQ initialization and maintenance tasks.
 * Triggers:
...
/**
 * Set environment variables for OMQ initialization
 */
export function setEnvironmentVariables(): string[] {
  ...
  if (process.env.CLAUDE_ENV_FILE) {
    try {
      const envContent = `export OMQ_INITIALIZED=true\n`;
      appendFileSync(process.env.CLAUDE_ENV_FILE, envContent);
      envVars.push('OMQ_INITIALIZED');
    } catch {
 ...
 * shells.  On Windows (MSYS2 / Git Bash) the same chain triggers Qoder CLI UI
 ...
 *
 * This fixes a silent breakage that occurs when OMQ upgrades to a new version:
...
export async function processSetupInit(input: SetupInput): Promise<HookOutput> {
  ...
  // The sh->find-node.sh->node chain triggers Qoder CLI UI bug #17088 on
...
/**
 * Prune old state files from .omq/state directory
 */
