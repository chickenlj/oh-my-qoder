...
function writeWorkflowLedger(
  ...
  for (const [skill, opts] of Object.entries(slots)) {
    active_skills[skill] = {
      ...
      initialized_state_path: join(tempDir, '.omq', 'state', 'skill-active-state.json'),
      initialized_session_state_path: join(tempDir, '.omq', 'state', 'sessions', sessionId, 'skill-active-state.json'),
    };
  }
...
function readSessionWorkflowLedger(tempDir: string, sessionId: string): {
  ...
  return JSON.parse(
    readFileSync(
      join(tempDir, '.omq', 'state', 'sessions', sessionId, 'skill-active-state.json'),
      'utf-8',
    ),
  );
}
...
function readRootWorkflowLedger(tempDir: string): {
  ...
  return JSON.parse(
    readFileSync(join(tempDir, '.omq', 'state', 'skill-active-state.json'), 'utf-8'),
  );
}
...
describe('workflow-gating: kill switches (spec i)', () => {
  ...
  beforeEach(() => {
    savedDisableOmq = process.env.DISABLE_OMQ;
    savedSkipHooks = process.env.OMQ_SKIP_HOOKS;
  });

  afterEach(() => {
    if (savedDisableOmq === undefined) {
      delete process.env.DISABLE_OMQ;
    } else {
      process.env.DISABLE_OMQ = savedDisableOmq;
    }
    if (savedSkipHooks === undefined) {
      delete process.env.OMQ_SKIP_HOOKS;
    } else {
      process.env.OMQ_SKIP_HOOKS = savedSkipHooks;
    }
  });

  it('DISABLE_OMQ=1 bypasses all stop gating', async () => {
  ...
  it('DISABLE_OMQ=true bypasses all stop gating', async () => {
    process.env.DISABLE_OMQ = 'true';
    const result = await checkPersistentModes('kill-sw-2', undefined);
    expect(result.shouldBlock).toBe(false);
  });

  it('OMQ_SKIP_HOOKS=persistent-mode bypasses stop gating', async () => {
    process.env.OMQ_SKIP_HOOKS = 'persistent-mode';
    const result = await checkPersistentModes('kill-sw-3', undefined);
    expect(result.shouldBlock).toBe(false);
    expect(result.mode).toBe('none');
  });

  it('OMQ_SKIP_HOOKS=stop-continuation bypasses stop gating', async () => {
    process.env.OMQ_SKIP_HOOKS = 'stop-continuation';
    const result = await checkPersistentModes('kill-sw-4', undefined);
    expect(result.shouldBlock).toBe(false);
  });

  it('OMQ_SKIP_HOOKS with comma-separated list bypasses when persistent-mode is included', async () => {
    process.env.OMQ_SKIP_HOOKS = 'some-hook,persistent-mode,other-hook';
