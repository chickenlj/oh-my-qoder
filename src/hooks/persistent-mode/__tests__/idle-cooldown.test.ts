...
const { TEST_HOME } = vi.hoisted(() => ({
  TEST_HOME: process.env.HOME || '/tmp/omq-test-home',
}));
...
const TEST_STATE_DIR = '/project/.omq/state';
...
function getConfigPaths(): string[] {
  return getGlobalOmqConfigCandidates('config.json');
}
...
describe('getIdleNotificationCooldownSeconds', () => {
  ...
  const originalOmqHome = process.env.OMQ_HOME;
  ...
  afterEach(() => {
    ...
    if (originalOmqHome === undefined) {
      delete process.env.OMQ_HOME;
    } else {
      process.env.OMQ_HOME = originalOmqHome;
    }
  });
  ...
  it('falls back to legacy ~/.omq config when XDG config is absent', () => {
