...
describe('skill-state', () => {
  ...
  describe('getSkillProtection', () => {
    ...
    it('returns none for instant/read-only skills', () => {
      ...
      expect(getSkillProtection('omq-help')).toBe('none');
      expect(getSkillProtection('omq-doctor')).toBe('none');
    });
    ...
    it('defaults to none for unknown/non-OMQ skills', () => {
      expect(getSkillProtection('unknown-skill')).toBe('none');
      expect(getSkillProtection('my-custom-skill')).toBe('none');
    });

    it('strips oh-my-qoder: prefix', () => {
      expect(getSkillProtection('oh-my-qoder:plan')).toBe('medium');
      expect(getSkillProtection('oh-my-qoder:ralph')).toBe('none');
    });
    ...
    it('returns none for project custom skills with same name as OMQ skills (issue #1581)', () => {
      // rawSkillName without oh-my-qoder: prefix → project custom skill
    ...
    it('returns protection for OMQ skills when rawSkillName has prefix', () => {
      expect(getSkillProtection('plan', 'oh-my-qoder:plan')).toBe('medium');
      expect(getSkillProtection('deepinit', 'oh-my-qoder:deepinit')).toBe('heavy');
    });

    it('returns none for other plugin skills with rawSkillName', () => {
      // ouroboros:plan, claude-mem:make-plan etc. should not get OMQ protection
  ...
  describe('writeSkillActiveState', () => {
    ...
    it('does not write state for unknown/custom skills', () => {
      ...
      expect(existsSync(join(tempDir, '.omq', 'state', 'sessions', 'session-1'))).toBe(false);
    });

    it('creates state file on disk', () => {
      writeSkillActiveState(tempDir, 'skill', 'session-1');
      const stateDir = join(tempDir, '.omq', 'state', 'sessions', 'session-1');
      const files = existsSync(stateDir);
      expect(files).toBe(true);
    });

    it('strips namespace prefix from skill name', () => {
      const state = writeSkillActiveState(tempDir, 'oh-my-qoder:plan', 'session-1');
      expect(state!.skill_name).toBe('plan');
    });

    it('does not write state for project custom skills with same name as OMQ skills (issue #1581)', () => {
    ...
    it('writes state for OMQ skills when rawSkillName has prefix', () => {
      const state = writeSkillActiveState(tempDir, 'plan', 'session-1', 'oh-my-qoder:plan');
      expect(state).not.toBeNull();
      expect(state!.skill_name).toBe('plan');
      expect(state!.max_reinforcements).toBe(5);
    });
    ...
    it('does not overwrite when mcp-setup is invoked inside omq-setup (canonical nesting scenario)', () => {
      writeSkillActiveState(tempDir, 'omq-setup', 'session-1');
      const child = writeSkillActiveState(tempDir, 'mcp-setup', 'session-1');
      expect(child).toBeNull();
      expect(readSkillActiveState(tempDir, 'session-1')!.skill_name).toBe('omq-setup');
    });
  ...
  describe('checkSkillActiveState', () => {
    ...
    it('clears stale state and allows stop', () => {
      ...
      const statePath = join(tempDir, '.omq', 'state', 'sessions', 'session-1', 'skill-active-state.json');
    ...
    it('nesting-aware clear: child completion preserves parent state, parent completion clears it', () => {
      // Simulates the full omq-setup → mcp-setup lifecycle including
      ...
      // "Verify stop hook no longer blocks after omq-setup completes with nested mcp-setup"

      // 1. Parent skill (omq-setup) starts
      writeSkillActiveState(tempDir, 'omq-setup', 'session-1');
      expect(readSkillActiveState(tempDir, 'session-1')!.skill_name).toBe('omq-setup');
      ...
      expect(parentState!.skill_name).toBe('omq-setup');
      ...
      expect(stopCheck.skillName).toBe('omq-setup');
      ...
      const completingParent = 'omq-setup';
  ...
  describe('writeSkillActiveStateCopies — dual-write invariant (spec a/b)', () => {
    const rootFilePath = (dir: string) => join(dir, '.omq', 'state', 'skill-active-state.json');
    const sessionFilePath = (dir: string, sid: string) =>
      join(dir, '.omq', 'state', 'sessions', sid, 'skill-active-state.json');
    ...
    it('writes only root copy when sessionId is omitted', () => {
      ...
      expect(existsSync(rootFilePath(tempDir))).toBe(true);
      expect(existsSync(join(tempDir, '.omq', 'state', 'sessions'))).toBe(false);
    });
  ...
  describe('readSkillActiveStateNormalized — normalization and session authority', () => {
    ...
    it('session copy is authoritative for session-local reads', () => {
      ...
      const rootDir = join(tempDir, '.omq', 'state');
  ...
  describe('upsertWorkflowSkillSlot — pure helper', () => {
    ...
    it('strips oh-my-qoder: prefix from skill name', () => {
      const state = upsertWorkflowSkillSlot(emptySkillActiveStateV2(), 'oh-my-qoder:ralph', {
        session_id: 's1',
        mode_state_path: 'r.json',
        initialized_mode: 'ralph',
        initialized_state_path: '',
      ...
      expect(state.active_skills['oh-my-qoder:ralph']).toBeUndefined();
    });
