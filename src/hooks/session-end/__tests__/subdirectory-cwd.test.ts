/**
 * Tests for issue #891: MCP state tools and stop hook resolve .omq/state/
 * differently when cwd is a subdirectory.
 *
 * processSessionEnd must normalize input.cwd to the git worktree root before
 * building any .omq/ paths, so it always operates on the same directory that
...
describe('processSessionEnd cwd normalization (issue #891)', () => {
  ...
  beforeEach(() => {
    worktreeRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'omq-891-root-'));
  ...
  it('reads and cleans up state written at worktree root, not subdirectory', async () => {
    ...
    const stateDir = path.join(worktreeRoot, '.omq', 'state');
  ...
  it('writes session summary to worktree root, not subdirectory', async () => {
    ...
    // Session summary should appear under worktreeRoot/.omq/sessions/
    const summaryPath = path.join(worktreeRoot, '.omq', 'sessions', 'test-session-891-summary.json');
    expect(fs.existsSync(summaryPath)).toBe(true);

    // Nothing should have been written under the subdirectory
    expect(fs.existsSync(path.join(subdirectory, '.omq'))).toBe(false);
  });
