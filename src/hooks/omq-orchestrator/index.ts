 ...
 *
 * Adapted from oh-my-opencode's omq-orchestrator hook for shell-based hooks.
 */
 ...
 * Checks: .omq/config.json → [$QODER_CONFIG_DIR|~/.qoder]/.omq-config.json → default (warn)
 */
function getEnforcementLevel(directory: string): EnforcementLevel {
  ...
  const localConfig = path.join(getOmqRoot(directory), 'config.json');
  const globalConfig = path.join(getQoderConfigDir(), '.omq-config.json');
...
export function processOrchestratorPreTool(input: ToolExecuteInput): ToolExecuteOutput {
  ...
  // Qoder CLI sends file_path (snake_case) for Write/Edit tools and notebook_path for NotebookEdit.
...
/**
 * Create omq orchestrator hook handlers
 */
export function createOmqOrchestratorHook(directory: string) {
  return {
    /**
