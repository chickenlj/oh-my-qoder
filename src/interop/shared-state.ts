 ...
 *
 * Manages shared state files at .omq/state/interop/ for communication
 * between OMQ (Qoder CLI) and OMX (Codex CLI).
 *
...
const SharedTaskSchema = z.object({
  id: z.string(),
  source: z.enum(['omq', 'omx']),
  target: z.enum(['omq', 'omx']),
...
export function getInteropDir(cwd: string): string {
  return join(getOmqRoot(cwd), 'state', 'interop');
}
...
export function initInteropSession(
  ...
  const interopDir = getInteropDir(omqCwd);
