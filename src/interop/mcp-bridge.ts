 ...
 *
 * Provides MCP tool definitions for communication between OMQ and OMX.
...
export function getInteropMode(env: NodeJS.ProcessEnv = process.env): InteropMode {
  const raw = (env.OMX_OMQ_INTEROP_MODE || 'off').toLowerCase();
...
export function canUseOmxDirectWriteBridge(env: NodeJS.ProcessEnv = process.env): boolean {
  const interopEnabled = env.OMX_OMQ_INTEROP_ENABLED === '1';
  const toolsEnabled = env.OMQ_INTEROP_TOOLS_ENABLED === '1';
...
function getInteropSource(target: 'omq' | 'omx'): 'omq' | 'omx' {
  return target === 'omq' ? 'omx' : 'omq';
}
...
export const interopSendTaskTool: ToolDefinition<{
  ...
  description: 'Send a task to the other tool (OMQ -> OMX or OMX -> OMQ) for execution. The task will be queued in shared state for the target tool to pick up.',
  schema: {
    target: z.enum(['omq', 'omx']).describe('Target tool to send the task to'),
...
export const interopReadResultsTool: ToolDefinition<{
  source: z.ZodOptional<z.ZodEnum<['omq', 'omx']>>;
  ...
  schema: {
    source: z.enum(['omq', 'omx']).optional().describe('Filter by source tool'),
  ...
  handler: async (args) => {
    ...
    try {
      ...
      const tasks = readSharedTasks(cwd, {
        source: source as 'omq' | 'omx' | undefined,
...
export const interopSendMessageTool: ToolDefinition<{
  ...
  schema: {
    target: z.enum(['omq', 'omx']).describe('Target tool to send the message to'),
...
export const interopSendOmxMessageTool: ToolDefinition<{
  ...
  schema: {
    ...
    fromWorker: z.string().describe('Sender worker name (e.g., "omq-bridge")'),
