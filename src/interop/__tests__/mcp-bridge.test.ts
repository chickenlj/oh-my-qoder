...
describe('interop mcp bridge gating', () => {
  it('getInteropMode normalizes invalid values to off', () => {
    expect(getInteropMode({ OMX_OMQ_INTEROP_MODE: 'ACTIVE' } as NodeJS.ProcessEnv)).toBe('active');
    expect(getInteropMode({ OMX_OMQ_INTEROP_MODE: 'observe' } as NodeJS.ProcessEnv)).toBe('observe');
    expect(getInteropMode({ OMX_OMQ_INTEROP_MODE: 'nonsense' } as NodeJS.ProcessEnv)).toBe('off');
  });

  it('canUseOmxDirectWriteBridge requires all active flags', () => {
    expect(canUseOmxDirectWriteBridge({
      OMX_OMQ_INTEROP_ENABLED: '1',
      OMX_OMQ_INTEROP_MODE: 'active',
      OMQ_INTEROP_TOOLS_ENABLED: '1',
    } as NodeJS.ProcessEnv)).toBe(true);

    expect(canUseOmxDirectWriteBridge({
      OMX_OMQ_INTEROP_ENABLED: '1',
      OMX_OMQ_INTEROP_MODE: 'observe',
      OMQ_INTEROP_TOOLS_ENABLED: '1',
    } as NodeJS.ProcessEnv)).toBe(false);

    expect(canUseOmxDirectWriteBridge({
      OMX_OMQ_INTEROP_ENABLED: '0',
      OMX_OMQ_INTEROP_MODE: 'active',
      OMQ_INTEROP_TOOLS_ENABLED: '1',
    } as NodeJS.ProcessEnv)).toBe(false);
  });
  ...
  it('interop_send_omx_message rejects when direct write path is disabled', async () => {
    const savedEnabled = process.env.OMX_OMQ_INTEROP_ENABLED;
    const savedMode = process.env.OMX_OMQ_INTEROP_MODE;
    const savedTools = process.env.OMQ_INTEROP_TOOLS_ENABLED;

    process.env.OMX_OMQ_INTEROP_ENABLED = '0';
    process.env.OMX_OMQ_INTEROP_MODE = 'off';
    process.env.OMQ_INTEROP_TOOLS_ENABLED = '0';

    try {
      const response = await interopSendOmxMessageTool.handler({
        teamName: 'alpha-team',
        fromWorker: 'omq-bridge',
        toWorker: 'worker-1',
        body: 'blocked',
      });
      ...
      if (savedEnabled === undefined) delete process.env.OMX_OMQ_INTEROP_ENABLED;
      else process.env.OMX_OMQ_INTEROP_ENABLED = savedEnabled;

      if (savedMode === undefined) delete process.env.OMX_OMQ_INTEROP_MODE;
      else process.env.OMX_OMQ_INTEROP_MODE = savedMode;

      if (savedTools === undefined) delete process.env.OMQ_INTEROP_TOOLS_ENABLED;
      else process.env.OMQ_INTEROP_TOOLS_ENABLED = savedTools;
    }
  });
});
