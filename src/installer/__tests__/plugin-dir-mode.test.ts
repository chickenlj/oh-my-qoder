/**
 * Tests for `--plugin-dir-mode` setup flag and `OMQ_PLUGIN_ROOT` auto-detection.
 *
 ...
 *   2. `OMQ_PLUGIN_ROOT` env var (set by `omq --plugin-dir`) → CLI auto-detects
 ...
 *   5. Real OMQ plugin enabled → existing skip behavior unchanged (independent
 *      of pluginDirMode).
 *
 * These tests run install() against a throwaway QODER_CONFIG_DIR and assert on
...
beforeEach(() => {
  testDir = mkdtempSync(join(tmpdir(), 'omq-pdm-'));
  // Force a clean, isolated config dir for every test
  process.env.QODER_CONFIG_DIR = testDir;
  // Avoid plugin auto-detection from the developer's real ~/.qwen
...
// CLI-level precedence rules (--plugin-dir-mode flag, OMQ_PLUGIN_ROOT
...
describe('5. real OMQ plugin enabled → existing skip behavior unchanged', () => {
  it('hasEnabledOmqPlugin() result drives skip independently of pluginDirMode', async () => {
