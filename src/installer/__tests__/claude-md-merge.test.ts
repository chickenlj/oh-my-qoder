...
describe('mergeClaudeMd', () => {
  const omqContent = '# OMQ Configuration\n\nThis is the OMQ content.';

  describe('Fresh install (no existing content)', () => {
    it('wraps omqContent in markers', () => {
      const result = mergeClaudeMd(null, omqContent);

      expect(result).toContain(START_MARKER);
      expect(result).toContain(END_MARKER);
      expect(result).toContain(omqContent);
      expect(result.indexOf(START_MARKER)).toBeLessThan(result.indexOf(omqContent));
      expect(result.indexOf(omqContent)).toBeLessThan(result.indexOf(END_MARKER));
    });
  ...
  describe('Update existing content with markers', () => {
    it('removes all marker blocks and preserves only user content outside them', () => {
      ...
      const result = mergeClaudeMd(existingContent, omqContent);
      ...
      expect(result).not.toContain('Old OMQ Content');
  ...
  describe('No markers in existing content', () => {
    it('wraps omqContent in markers and preserves existing content after user customizations header', () => {
      ...
      // Check order: OMQ section first, then user customizations header, then existing content
      const omqIndex = result.indexOf(START_MARKER);
      const customizationsIndex = result.indexOf(USER_CUSTOMIZATIONS);
      const existingIndex = result.indexOf('# My Custom Config');

      expect(omqIndex).toBeLessThan(customizationsIndex);
  ...
  describe('Corrupted markers', () => {
    ...
    it('does not grow unboundedly when called repeatedly with corrupted markers', () => {
      ...
      const firstResult = mergeClaudeMd(corruptedContent, omqContent);

      // Call again with the output of the first call
      const secondResult = mergeClaudeMd(firstResult, omqContent);
      ...
      const thirdResult = mergeClaudeMd(secondResult, omqContent);
      expect(thirdResult.length).toBeLessThanOrEqual(secondResult.length * 1.1);
    });

    it('strips unmatched OMQ markers from recovered content', () => {
      const corruptedContent = `${START_MARKER}\nUser custom config`;
      const result = mergeClaudeMd(corruptedContent, omqContent);

      // The recovered section should not contain bare OMQ markers
      // Count occurrences of START_MARKER: should only appear once (in the OMQ block)
  ...
  describe('Edge cases', () => {
    ...
    it('handles multi-line omqContent', () => {
      const multiLineOmq = 'Line 1\nLine 2\nLine 3\n\nLine 5';
      const result = mergeClaudeMd(null, multiLineOmq);

      expect(result).toContain(multiLineOmq);
...
- Follow company coding standards`;
      ...
      const newOmqContent = '# OMQ Config v2.0\nNew instructions with updates.';
      const result = mergeClaudeMd(existingContent, newOmqContent);

      expect(result).toContain('# OMQ Config v2.0');
...
User added custom stuff here`;
      ...
      const result = mergeClaudeMd(oldContent, omqContent);

      // New OMQ content should be at the top with markers
  ...
  describe('idempotency guard', () => {
    it('strips markers from omqContent that already has markers', () => {
      // Simulate docs/CLAUDE.md shipping with markers already
      const omqWithMarkers = `<!-- OMQ:START -->
# oh-my-qoder
Agent instructions here
<!-- OMQ:END -->`;

      const result = mergeClaudeMd(null, omqWithMarkers);
    ...
    it('handles omqContent with markers when merging into existing content', () => {
...
<!-- OMQ:END -->`;
      ...
      const result = mergeClaudeMd(existingContent, omqWithMarkers);
      ...
      expect(startCount).toBe(1);
      expect(endCount).toBe(1);
      expect(result).toContain('New OMQ content v2');
      expect(result).not.toContain('Old OMQ content');
  ...
  describe('version marker sync', () => {
    it('injects the provided version marker on fresh install', () => {
      const result = mergeClaudeMd(null, omqContent, '4.6.7');
...
my notes`;
      ...
      const result = mergeClaudeMd(existingContent, omqContent, '4.6.7');
    ...
    it('strips embedded version marker from omq content before inserting current version', () => {
      const omqWithVersion = `<!-- OMQ:VERSION:4.0.0 -->\n${omqContent}`;

      const result = mergeClaudeMd(null, omqWithVersion, '4.6.7');
  ...
  describe('issue #1467 regression', () => {
    it('removes duplicate legacy OMQ blocks from preserved user content', () => {
      const existingContent = `${START_MARKER}
Old OMQ content v1
${END_MARKER}
...
My note after duplicate block`;
      ...
      expect(result).not.toContain('Old OMQ content v1');
