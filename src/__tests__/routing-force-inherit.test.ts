 ...
 * model instead of using OMQ's per-agent model routing.
 */
...
describe('routing.forceInherit (issue #1135)', () => {
  ...
  beforeEach(() => {
    originalEnv = process.env.OMQ_ROUTING_FORCE_INHERIT;
    vi.clearAllMocks();
  });
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.OMQ_ROUTING_FORCE_INHERIT = originalEnv;
    }
  });
  ...
  describe('routeTask with forceInherit', () => {
    it('returns inherit model type when forceInherit is true', () => {
      const result = routeTask(
        ...
        { enabled: true, defaultTier: 'MEDIUM', forceInherit: true, escalationEnabled: false, maxEscalations: 0, tierModels: { LOW: 'low', MEDIUM: 'medium', HIGH: 'high' } }
      );
    ...
    it('bypasses agent-specific overrides when forceInherit is true', () => {
      const result = routeTask(
        { taskPrompt: 'Design system architecture', agentType: 'architect' },
        {
          ...
          maxEscalations: 0,
          tierModels: { LOW: 'low', MEDIUM: 'medium', HIGH: 'high' },
          agentOverrides: {
    ...
    it('routes normally when forceInherit is false', () => {
      const result = routeTask(
        { taskPrompt: 'Find all files', agentType: 'explore' },
        { enabled: true, defaultTier: 'MEDIUM', forceInherit: false, escalationEnabled: false, maxEscalations: 0, tierModels: { LOW: 'low', MEDIUM: 'medium', HIGH: 'high' } }
      );
    ...
    it('routes normally when forceInherit is undefined', () => {
      const result = routeTask(
        { taskPrompt: 'Find all files', agentType: 'explore' },
        { enabled: true, defaultTier: 'MEDIUM', escalationEnabled: false, maxEscalations: 0, tierModels: { LOW: 'low', MEDIUM: 'medium', HIGH: 'high' } }
      );
  ...
  describe('getModelForTask with forceInherit', () => {
    it('returns inherit for all agent types when forceInherit is true', () => {
      const config = { enabled: true, defaultTier: 'MEDIUM' as const, forceInherit: true, escalationEnabled: false, maxEscalations: 0, tierModels: { LOW: 'low', MEDIUM: 'medium', HIGH: 'high' } };
  ...
  describe('processPreToolUse with forceInherit', () => {
    it('strips model from Task calls when forceInherit is true', () => {
      ...
      expect(modified.subagent_type).toBe('oh-my-qoder:executor');
    });
    ...
    it('does not strip model when forceInherit is false', () => {
      ...
      expect(modified.model).toBe('low');
    });
