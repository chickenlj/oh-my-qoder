...
describe('Builtin Skills', () => {
  ...
  describe('getBuiltinSkill()', () => {
    ...
    it('should keep ask as the canonical process-first advisor wrapper', () => {
      ...
      expect(skill?.template).toContain('omq ask {{ARGUMENTS}}');
    ...
    it('loads deep-interview ambiguityThreshold source before state init and updates the first-line marker', () => {
      ...
      writeFileSync(
        ...
        JSON.stringify({ omq: { deepInterview: { ambiguityThreshold: 0.15 } } }),
      );

      mkdirSync(join(projectDir, '.claude'), { recursive: true });
      writeFileSync(
        join(projectDir, '.claude', 'settings.json'),
        JSON.stringify({ omq: { deepInterview: { ambiguityThreshold: 0.12 } } }),
      );
    ...
    it('refreshes cached deep-interview output when the configured threshold changes without requiring manual cache clearing', () => {
      const projectDir = mkdtempSync(join(tmpdir(), 'omq-skill-cache-refresh-'));
      ...
      writeFileSync(
        join(projectDir, '.claude', 'settings.json'),
        JSON.stringify({ omq: { deepInterview: { ambiguityThreshold: 0.33 } } }),
      );
    ...
    it('replaces all hardcoded 20%/0.2 threshold references in deep-interview template (issue #2545)', () => {
      const profileDir = mkdtempSync(join(tmpdir(), 'omq-skill-2545-'));
    ...
    it('ships a config-aware deep-interview SKILL.md for native skill-loader paths (issues #2723, #3030)', () => {
      ...
      expect(raw).toContain('Preserve the AskUserQuestion path for OMQ-native interaction');
    ...
    it('applies deep-interview runtime settings for plugin-qualified rendered skill names (issue #3030)', () => {
      const profileDir = mkdtempSync(join(tmpdir(), 'omq-skill-3030-'));
      tempDirs.push(profileDir);

      process.env.QODER_CONFIG_DIR = profileDir;
      writeFileSync(
        join(profileDir, 'settings.json'),
        JSON.stringify({ omq: { deepInterview: { ambiguityThreshold: 0.17 } } }),
      );
      clearSkillsCache();
    ...
    it('loads deep-dive ambiguityThreshold from deep-interview settings before state init and updates threshold copy', () => {
      const profileDir = mkdtempSync(join(tmpdir(), 'omq-deep-dive-profile-'));
      const projectDir = mkdtempSync(join(tmpdir(), 'omq-deep-dive-project-'));
      tempDirs.push(profileDir, projectDir);
      ...
      writeFileSync(
        join(projectDir, '.claude', 'settings.json'),
        JSON.stringify({ omq: { deepInterview: { ambiguityThreshold: 0.11 } } }),
      );
      ...
      expect(t).toContain('Resolve `omq.deepInterview.ambiguityThreshold` into `0.11`');
      ...
      expect(t).not.toContain('omq.deepDive.ambiguityThreshold');
    });
    ...
    it('ships config-aware deep-dive SKILL.md using the deep-interview threshold namespace', () => {
      ...
      expect(raw).toContain('`.omq/specs/deep-dive-trace-{slug}.md`');
      expect(raw).toContain('`.omq/specs/deep-dive-{slug}.md`');
      expect(raw).toContain('`.omq/state/` or `state_write` for ephemeral artifacts');

      expect(raw).not.toContain('omq.deepDive.ambiguityThreshold');
      expect(raw).not.toContain('"threshold": 0.2,');
      expect(raw).not.toContain('Gate: <=20% ambiguity');
      expect(raw).not.toContain('ambiguity <= 20%');
    });
    ...
    it('renders deep-interview summary-gate hardening while preserving AskUserQuestion transport', () => {
      ...
      expect(t).toContain('Preserve the AskUserQuestion path for OMQ-native interaction');
      expect(t).toContain('Initial Context Summarized: {yes|no}');
      expect(t).not.toContain('omx question');
    });

    it('rewrites built-in skill command examples to plugin-safe bridge invocations when omq is unavailable', () => {
      process.env.QODER_PLUGIN_ROOT = '/plugin-root';
      process.env.PATH = '';
      // Simulate a non-Claude-session context: the ask-skill rewriter only keeps
      // `omq ask` form when running *inside* an active Claude session, so we must
      // clear the session-detection vars that may leak in from the test runner.
      const savedClaudeCode = process.env.CLAUDECODE;
      const savedSessionId = process.env.QODER_SESSION_ID;
      ...
      try {
        ...
        if (savedSessionId === undefined) delete process.env.QODER_SESSION_ID;
        else process.env.QODER_SESSION_ID = savedSessionId;
    ...
    it('should require explicit tmux prerequisite checks for omq-teams', () => {
      const skill = getBuiltinSkill('omq-teams');
      expect(skill).toBeDefined();
    ...
    it('should document allowed omq-teams agent types and native team fallback', () => {
      const skill = getBuiltinSkill('omq-teams');
      expect(skill).toBeDefined();
      expect(skill?.template).toContain('/omq-teams` only supports **`claude`**, **`codex`**, and **`gemini`**');
      expect(skill?.template).toContain('unsupported type such as `expert`');
      expect(skill?.template).toContain('/oh-my-qoder:team');
    });

    it('should preserve the multi-repo omq-teams cwd and plan-path contract', () => {
      const skill = getBuiltinSkill('omq-teams');
      expect(skill).toBeDefined();
      ...
      expect(skill?.template).toContain('Do not anchor the launch cwd to only the repo containing `.omq/plans/...`');
