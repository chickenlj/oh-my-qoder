...
describe('setup-progress.sh', () => {
  it('writes setup completion metadata to QODER_CONFIG_DIR', () => {
    const root = mkdtempSync(join(tmpdir(), 'omq-setup-progress-'));
    tempRoots.push(root);
    ...
    const configPath = join(configDir, '.omq-config.json');
  ...
  it('fails without jq and preserves existing setup config', () => {
    const root = mkdtempSync(join(tmpdir(), 'omq-setup-progress-no-jq-'));
    tempRoots.push(root);
