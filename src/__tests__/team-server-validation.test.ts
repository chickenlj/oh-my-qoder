...
const VALID_JOB_ID_RE = /^omq-[a-z0-9]{1,16}$/;
...
describe('validateJobId', () => {
  describe('rejects path traversal and invalid inputs', () => {
    const traversalPayloads = [
      ...
      'omq-UPPERCASE',
      'omq-has spaces',
      'omq-' + 'a'.repeat(17), // 17 chars — exceeds 16-char limit
      'notprefixed',
      'omq_underscore',
      'omq-abc!@#',
    ];
  ...
  describe('accepts valid job IDs', () => {
    const validIds = [
      'omq-abc123',
      'omq-a',
      'omq-123456789012', // 12 chars
      'omq-1',
      'omq-abcdefghijkl', // 12 lowercase letters
      'omq-abcdefghijklmnop', // exactly 16 chars
    ];
...
describe('team-server handler validation integration', () => {
  ...
  it('production validateJobId regex matches test regex', async () => {
    ...
    expect(src).toContain('/^omq-[a-z0-9]{1,16}$/');
  });
