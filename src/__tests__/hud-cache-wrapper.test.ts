...
describe('HUD cache wrapper stale render cleanup', () => {
  it('removes stale render locks and zero-byte temp files without deleting diagnostics', () => {
    const tempRoot = mkdtempSync(join(tmpdir(), 'omq-hud-cache-wrapper-'));
    ...
    const output = execFileSync('sh', [wrapperPath, hudScript], {
      ...
      env: {
        ...process.env,
        OMQ_HUD_CACHE_DIR: cacheDir,
        OMQ_HUD_SYNC_REFRESH: '1',
      },
    });

    // First render is synchronous when stdin is available so Qoder CLI v2.1.x
