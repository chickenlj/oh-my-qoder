...
// Mock getOmqRoot to use our test directory
...
describe('Shared Memory', () => {
  ...
  beforeEach(() => {
    ...
    tildeConfigDir = join(homedir(), `.omq-test-shared-memory-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  describe('writeEntry / readEntry', () => {
    ...
    it('should create namespace directory automatically', () => {
      ...
      const nsDir = join(omqDir, 'state', 'shared-memory', 'auto-ns');
      expect(existsSync(nsDir)).toBe(true);
    });

    it('should store entry as JSON file', () => {
      writeEntry('ns', 'mykey', { x: 1 });
      const filePath = join(omqDir, 'state', 'shared-memory', 'ns', 'mykey.json');
  ...
  describe('TTL support', () => {
    ...
    it('should auto-delete expired entries on read', () => {
      ...
      const filePath = join(omqDir, 'state', 'shared-memory', 'ns');
  ...
  describe('listEntries', () => {
    ...
    it('should filter out expired entries', () => {
      ...
      const nsDir = join(omqDir, 'state', 'shared-memory', 'ns');
      const expiredEntry = {
        key: 'dead',
        value: 'expired',
        namespace: 'ns',
  ...
  describe('isSharedMemoryEnabled', () => {
    ...
    it('should read config from the active QODER_CONFIG_DIR', () => {
      const claudeConfigDir = join(testDir, 'claude-config');
      mkdirSync(claudeConfigDir, { recursive: true });
      writeFileSync(join(claudeConfigDir, '.omq-config.json'), JSON.stringify({
        agents: {
          sharedMemory: {
            enabled: false,
          },
    ...
    it('should expand ~-prefixed QODER_CONFIG_DIR values', () => {
      mkdirSync(tildeConfigDir, { recursive: true });
      writeFileSync(join(tildeConfigDir, '.omq-config.json'), JSON.stringify({
        agents: {
          sharedMemory: {
            enabled: false,
          },
  ...
  describe('atomic writes', () => {
    it('should not leave temp file after successful write', () => {
      ...
      const filePath = join(omqDir, 'state', 'shared-memory', 'ns', 'clean-test.json');
      expect(existsSync(filePath)).toBe(true);
    ...
    it('should preserve original file when a leftover .tmp exists from a prior crash', () => {
      ...
      const filePath = join(omqDir, 'state', 'shared-memory', 'ns', 'crash-test.json');
