...
describe('wiki hook wrapper output', () => {
  ...
  beforeEach(() => {
    ...
    writeFileSync(
      join(tempDir, '.omq', 'wiki', 'test-page.md'),
      [
        '---',
        'title: "Test Page"',
        'tags: ["test"]',
    ...
    writeFileSync(join(tempDir, '.omq', 'wiki', 'index.md'), '# Wiki Index\n- test-page.md\n');
  });
  ...
  it('wraps SessionStart wiki context under hookSpecificOutput', () => {
    ...
    expect(output.hookSpecificOutput).toEqual({
      ...
      additionalContext: expect.stringContaining('[LLM Wiki: 1 pages at .omq/wiki/]'),
    });
  });
