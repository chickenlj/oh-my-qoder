...
describe('delegation-enforcement-levels', () => {
  ...
  describe('processOrchestratorPreTool enforcement levels', () => {
    function setEnforcement(level: string) {
      mockExistsSync.mockImplementation((p: unknown) => {
        const s = String(p);
        if (/[\\/]\.omq[\\/]config\.json$/.test(s)) return true;
        return false;
      });
      mockReadFileSync.mockImplementation(() => {
  ...
  describe('isAllowedPath', () => {
    it('returns true for .omq/ paths', () => {
      expect(isAllowedPath('.omq/plans/test.md')).toBe(true);
    });
    ...
    it('rejects .omq/../src/file.ts traversal', () => {
    ...
    it('handles Windows-style .omq paths', () => {
      expect(isAllowedPath('.omq\\plans\\test.md')).toBe(true);
    });

    it('rejects Windows traversal .omq\\..\\src\\file.ts', () => {
      expect(isAllowedPath('.omq\\..\\src\\file.ts')).toBe(false);
    });

    // Nested .omq in non-root position (should be rejected for relative paths)
    it('rejects foo/.omq/bar.ts as relative path', () => {
      expect(isAllowedPath('foo/.omq/bar.ts')).toBe(false);
    });

    // Windows mixed-separator edge cases
    it('rejects mixed separator traversal .omq\\..\\..\\secret', () => {
      expect(isAllowedPath('.omq\\..\\..\\secret')).toBe(false);
    });

    it('rejects double-dot with mixed separators .omq/..\\src', () => {
      expect(isAllowedPath('.omq/..\\src')).toBe(false);
    });

    it('rejects UNC paths as not relative to project', () => {
      expect(isAllowedPath('\\\\server\\share\\.omq\\file')).toBe(false);
    });

    it('rejects absolute Windows drive paths without worktree root', () => {
      expect(isAllowedPath('C:\\repo\\.omq\\file')).toBe(false);
    });
  });
