...
describe('Tier-0 contract docs consistency', () => {
  ...
  it.skip('uses the published /docs/ path instead of the removed docs.html path in README links (translated READMEs not present)', () => {
    const readmes = [
      'README.md',
      'README.de.md',
      'README.es.md',
  ...
  it('keeps root AGENTS.md aligned with OMQ branding and state paths', () => {
    const agentsDoc = readProjectFile('AGENTS.md');

    expect(agentsDoc).toContain('# oh-my-qoder - Intelligent Multi-Agent Orchestration');
    expect(agentsDoc).toContain('You are running with oh-my-qoder (OMQ), a multi-agent orchestration layer for Qoder CLI.');
    expect(agentsDoc).toContain('`.omq/state/`');
    expect(agentsDoc).toContain('Run `omq setup` to install all components. Run `omq doctor` to verify installation.');
  ...
  it.skip('keeps benchmark default model references aligned across docs and scripts (benchmark dir not present)', () => {
    ...
    const omq = readProjectFile('benchmark', 'run_omq.sh');
    ...
    const expectedModel = 'qwen-plus-20260217';

    for (const content of [benchmarkReadme, benchmarkRunner, quickTest, vanilla, omq, fullComparison, resultsReadme]) {
      expect(content).toContain(expectedModel);
    }

    expect(benchmarkReadme).not.toContain('claude-sonnet-4.5-20250929');
    expect(benchmarkRunner).not.toContain('qwen-plus0250514');
    expect(resultsReadme).toContain('Qwen Plus');
  });

  it.skip('removes dead package build aliases and keeps seminar demo model guidance current (seminar dir not present)', () => {
    ...
    expect(seminarDemo).toContain('# 빠른 모델 (Plus)');
    expect(seminarDemo).toContain('export OMQ_MODEL=anthropic/qwen-plus');
    expect(seminarDemo).not.toContain('anthropic/qwen-plus');
  });
});
