...
describe('standalone-server tool composition', () => {
  ...
  it('should include load_omq_skills_local tool', () => {
    const names = skillsTools.map(t => t.name);
    expect(names).toContain('load_omq_skills_local');
  });

  it('should include list_omq_skills tool', () => {
    const names = skillsTools.map(t => t.name);
    expect(names).toContain('list_omq_skills');
  });
