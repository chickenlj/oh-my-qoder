...
describe("Qoder CLI /goal adapter docs contract", () => {
  ...
  it("documents Claude/Anthropic as the only authority for Qoder CLI /goal facts", () => {
    expect(adapterDoc).toContain("https://code.claude.com/docs/en/goal");
    expect(adapterDoc).toContain(
      "https://raw.githubusercontent.com/anthropics/claude-code/main/CHANGELOG.md",
    );
    expect(adapterDoc).toContain(
      "they are not authority for Qoder CLI `/goal` facts",
    );
  });

  it("documents the hidden-state non-mutation boundary", () => {
    expect(adapterDoc).toContain(
      "it does not mutate hidden Qoder CLI goal state",
    );
    expect(adapterDoc).toContain(
      "instead of writing hidden Qoder CLI session state directly",
    );
    expect(referenceDoc).toContain(
      "it must not mutate hidden Qoder CLI session state directly",
    );
  });
  ...
  it("keeps evaluator success separate from OMQ final completion", () => {
  ...
  it("links the adapter design from REFERENCE.md", () => {
    expect(referenceDoc).toContain(
      "[Qoder CLI `/goal` Adapter Design](#claude-code-goal-adapter-design)",
    );
