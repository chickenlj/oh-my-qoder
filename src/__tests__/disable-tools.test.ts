/**
 * Tests for OMQ_DISABLE_TOOLS env var support
 *
...
import { parseDisabledGroups, DISABLE_TOOLS_GROUP_MAP } from '../mcp/omq-tools-server.js';
import { TOOL_CATEGORIES } from '../constants/index.js';

describe('OMQ_DISABLE_TOOLS', () => {
  let savedEnv: string | undefined;

  beforeEach(() => {
    savedEnv = process.env.OMQ_DISABLE_TOOLS;
    delete process.env.OMQ_DISABLE_TOOLS;
  });

  afterEach(() => {
    if (savedEnv !== undefined) {
      process.env.OMQ_DISABLE_TOOLS = savedEnv;
    } else {
      delete process.env.OMQ_DISABLE_TOOLS;
    }
  });
  ...
  describe('parseDisabledGroups()', () => {
    ...
    describe('robustness', () => {
      ...
      it('reads from process.env.OMQ_DISABLE_TOOLS when no argument given', () => {
        process.env.OMQ_DISABLE_TOOLS = 'lsp,ast';
        const result = parseDisabledGroups();
        expect(result.has(TOOL_CATEGORIES.LSP)).toBe(true);
        expect(result.has(TOOL_CATEGORIES.AST)).toBe(true);
      });
      ...
      it('explicit argument takes precedence over env var', () => {
        process.env.OMQ_DISABLE_TOOLS = 'lsp';
        const result = parseDisabledGroups('ast');
        expect(result.has(TOOL_CATEGORIES.AST)).toBe(true);
