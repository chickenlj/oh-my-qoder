...
describe('cleanupTransientState — session-scoped hud-stdin-cache', () => {
  ...
  beforeEach(() => {
    tmpRoot = mkdtempSync(join(tmpdir(), 'omq-session-end-cleanup-'));
  });
  ...
  it('removes the ending session\'s hud-stdin-cache.json and prunes its empty directory', () => {
    ...
    const sessionDir = join(tmpRoot, '.omq', 'state', 'sessions', 'session-aaa');
  ...
  it('preserves the ending session\'s dir when it still has non-transient state', () => {
    const sessionDir = join(tmpRoot, '.omq', 'state', 'sessions', 'session-bbb');
    mkdirSync(sessionDir, { recursive: true });
  ...
  it('does not delete another running session\'s hud-stdin-cache.json', () => {
    const ending = join(tmpRoot, '.omq', 'state', 'sessions', 'session-ending');
    const other = join(tmpRoot, '.omq', 'state', 'sessions', 'session-other');
