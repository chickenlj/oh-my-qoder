...
describe('Qoder CLI compatibility for OMQ-authored user skills', () => {
  let tempDir: string;
  let originalClaudeConfigDir: string | undefined;

  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'omq-user-skill-compat-'));
  ...
  it('reproduces that nested omq-learned skills are invisible to flat Qoder CLI lookup until compat sync runs', async () => {
    const nestedSkillPath = join(tempDir, 'skills', 'omq-learned', 'expert-review', 'SKILL.md');
    ...
    const { syncOmqLearnedUserSkillsForClaudeCode } = await import('../utils/user-skill-compat.js');
    expect(syncOmqLearnedUserSkillsForClaudeCode()).toEqual(['expert-review']);

    expect(existsSync(flatSkillPath)).toBe(true);
    expect(readFileSync(flatSkillPath, 'utf-8')).toContain('Use expert review.');
  });

  it('keeps new user skills in omq-learned/<name>.md and exposes a flat Qoder CLI SKILL.md shim', async () => {
    ...
    expect(result.path).toBe(join(tempDir, 'skills', 'omq-learned', 'expert-review.md'));
  ...
  it('does not overwrite an existing user-authored flat skill directory with the same name', async () => {
    ...
    const { syncOmqLearnedUserSkillsForClaudeCode } = await import('../utils/user-skill-compat.js');
    expect(syncOmqLearnedUserSkillsForClaudeCode()).toEqual([]);
    expect(readFileSync(flatSkillPath, 'utf-8')).toContain('User flat skill.');
  });

  it('uses a symlink shim when supported so edits to the omq-learned source remain visible', async () => {
    const nestedSkillPath = join(tempDir, 'skills', 'omq-learned', 'expert-review', 'SKILL.md');
    mkdirSync(join(nestedSkillPath, '..'), { recursive: true });
    writeFileSync(nestedSkillPath, '---\nname: expert-review\ntriggers: [expert-review]\n---\n\nBefore edit.\n');
