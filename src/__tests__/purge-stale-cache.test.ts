...
vi.mock('../utils/config-dir.js', () => ({
  getQoderConfigDir: vi.fn(() => '/mock/.qwen'),
}));
...
describe('purgeStalePluginCacheVersions', () => {
  ...
  it('removes stale versions not in installed_plugins.json', () => {
    const cacheDir = '/mock/.qwen/plugins/cache';
  ...
  it('handles multiple marketplaces and plugins', () => {
    ...
    const active2 = join(cacheDir, 'omq/oh-my-qoder/4.3.0');
    ...
    mockedReadFileSync.mockReturnValue(JSON.stringify({
      ...
      plugins: {
        ...
        'oh-my-qoder@omq': [{ installPath: active2 }],
      },
    }));

    mockedReaddirSync.mockImplementation((p, _opts?) => {
      const ps = String(p);
      if (ps === cacheDir) return [dirent('official'), dirent('omq')] as any;
      ...
      if (ps.endsWith('omq')) return [dirent('oh-my-qoder')] as any;
      if (ps.endsWith('oh-my-qoder')) return [dirent('4.3.0')] as any;
      return [] as any;
    });
  ...
  it('does nothing when all cache versions are active', () => {
    const cacheDir = '/mock/.qwen/plugins/cache';
    const active = join(cacheDir, 'omq/oh-my-qoder/4.3.0');
    ...
    mockedReadFileSync.mockReturnValue(JSON.stringify({
      version: 2,
      plugins: {
        'oh-my-qoder@omq': [{ installPath: active }],
      },
    }));

    mockedReaddirSync.mockImplementation((p, _opts?) => {
      const ps = String(p);
      if (ps === cacheDir) return [dirent('omq')] as any;
      if (ps.endsWith('omq')) return [dirent('oh-my-qoder')] as any;
      if (ps.endsWith('oh-my-qoder')) return [dirent('4.3.0')] as any;
      return [] as any;
    });
  ...
  it('matches installPath with trailing slash correctly', () => {
    const cacheDir = '/mock/.qwen/plugins/cache';
    const versionDir = join(cacheDir, 'omq/plugin/1.0.0');
    ...
    mockedReaddirSync.mockImplementation((p, _opts?) => {
      const ps = String(p);
      if (ps === cacheDir) return [dirent('omq')] as any;
      if (ps.endsWith('omq')) return [dirent('plugin')] as any;
  ...
  it('preserves version when installPath points to a subdirectory', () => {
    const cacheDir = '/mock/.qwen/plugins/cache';
    const versionDir = join(cacheDir, 'omq/plugin/2.0.0');
  ...
  function setupFreshNonActiveCache() {
    ...
    mockedReadFileSync.mockReturnValue(JSON.stringify({
      version: 2,
      plugins: { 'plugin@omq': [{ installPath: '/other/path' }] },
    }));
    mockedReaddirSync.mockImplementation((p, _opts?) => {
      const ps = String(p);
      if (ps === cacheDir) return [dirent('omq')] as any;
  ...
  it('replaces stale version dir with symlink to active version in same namespace', () => {
    ...
    const cacheDir = '/mock/.qwen/plugins/cache';
    const activeVersion = join(cacheDir, 'omq/oh-my-qoder/4.14.5');
    const staleVersion = join(cacheDir, 'omq/oh-my-qoder/4.14.4');
    ...
    mockedReadFileSync.mockReturnValue(JSON.stringify({
      version: 2,
      plugins: {
        'oh-my-qoder@omq': [{ installPath: activeVersion, version: '4.14.5' }],
      },
    }));
    ...
    mockedReaddirSync.mockImplementation((p, _opts?) => {
      ...
      if (ps === cacheDir) return [dirent('omq')] as any;
      if (ps.endsWith('omq')) return [dirent('oh-my-qoder')] as any;
      if (ps.endsWith('oh-my-qoder')) return [dirent('4.14.4'), dirent('4.14.5')] as any;
      return [] as any;
    });
  ...
  it('deletes stale version dir when no active version exists in namespace', () => {
    ...
    const cacheDir = '/mock/.qwen/plugins/cache';
    const staleVersion = join(cacheDir, 'omq/plugin/1.0.0');

    mockedExistsSync.mockReturnValue(true);
    mockedReadFileSync.mockReturnValue(JSON.stringify({
      version: 2,
      plugins: {
        // installPath is outside the omq/plugin namespace
  ...
  it('skips version directory entries where isDirectory() returns false (existing symlinks)', () => {
    ...
    mockedReadFileSync.mockReturnValue(JSON.stringify({
      version: 2,
      plugins: {
        'oh-my-qoder@omq': [{ installPath: activeVersion }],
      },
    }));
