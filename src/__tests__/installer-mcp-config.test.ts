...
async function loadInstallerWithEnv(claudeConfigDir: string, homeDir: string, codexHome: string, omqHome: string) {
  vi.resetModules();
  process.env.QODER_CONFIG_DIR = claudeConfigDir;
  process.env.HOME = homeDir;
  process.env.CODEX_HOME = codexHome;
  process.env.OMQ_HOME = omqHome;
...
describe('installer MCP config ownership (issue #1802)', () => {
  ...
  beforeEach(() => {
    tempRoot = mkdtempSync(join(tmpdir(), 'omq-installer-mcp-config-'));
    homeDir = join(tempRoot, 'home');
    claudeConfigDir = join(homeDir, '.qwen');
    codexHome = join(tempRoot, '.codex');
    omqHome = join(tempRoot, '.omq');

    mkdirSync(homeDir, { recursive: true });
    mkdirSync(claudeConfigDir, { recursive: true });
    mkdirSync(codexHome, { recursive: true });
    mkdirSync(omqHome, { recursive: true });
  ...
  it('moves legacy settings.json mcpServers into ~/.qoder.json during install', async () => {
    ...
    const registryPath = join(omqHome, 'mcp-registry.json');
    ...
    const installer = await loadInstallerWithEnv(claudeConfigDir, homeDir, codexHome, omqHome);
