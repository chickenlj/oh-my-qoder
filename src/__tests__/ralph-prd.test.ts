...
describe('Ralph PRD Module', () => {
  ...
  describe('findPrdPath', () => {
    ...
    it('should find prd.json in .omq directory', () => {
      const omqDir = join(testDir, '.omq');
      mkdirSync(omqDir, { recursive: true });
      const prdPath = join(omqDir, PRD_FILENAME);
      writeFileSync(prdPath, '{}');
      expect(findPrdPath(testDir)).toBe(prdPath);
    });

    it('should prefer root over .omq', () => {
      const rootPath = join(testDir, PRD_FILENAME);
      const omqDir = join(testDir, '.omq');
      mkdirSync(omqDir, { recursive: true });
      const omqPath = join(omqDir, PRD_FILENAME);

      writeFileSync(rootPath, '{"source": "root"}');
      writeFileSync(omqPath, '{"source": "omq"}');
  ...
  describe('readPrd / writePrd', () => {
    ...
    it('should create .omq directory when writing', () => {
      writePrd(testDir, samplePrd);
      expect(existsSync(join(testDir, '.omq'))).toBe(true);
    });
    ...
    it('isolates transient PRDs for concurrent sessions in the same project', () => {
      ...
      expect(existsSync(join(testDir, '.omq', 'prd.json'))).toBe(false);
    });
  ...
  describe('markStoryComplete / markStoryIncomplete', () => {
    ...
    it('should return false when no prd exists', () => {
      rmSync(join(testDir, '.omq'), { recursive: true, force: true });
