...
describe('isZaiHost', () => {
  ...
  it('rejects unrelated hosts', () => {
    expect(isZaiHost('https://api.dashscope.aliyuncs.com')).toBe(false);
...
describe('getUsage routing', () => {
  ...
  const expectedServiceName = (configDir: string) =>
    `Qoder CLI-credentials-${createHash('sha256').update(configDir).digest('hex').slice(0, 8)}`;
  ...
  beforeEach(async () => {
    ...
    delete process.env.DASHSCOPE_AUTH_TOKEN;
  ...
  it('uses the raw ~-prefixed QODER_CONFIG_DIR value for Keychain service lookup', async () => {
    process.env.QODER_CONFIG_DIR = '~/.qwen-personal';
  ...
  it('uses a different Keychain service when QODER_CONFIG_DIR is already expanded', async () => {
    process.env.QODER_CONFIG_DIR = '/Users/test/.qwen-personal';
  ...
  it('routes z.ai Anthropic Messages endpoint to the quota API', async () => {
    process.env.DASHSCOPE_BASE_URL = 'https://api.z.ai/api/anthropic';
    process.env.DASHSCOPE_AUTH_TOKEN = 'test-token';
  ...
  it('does NOT route to z.ai for look-alike hosts', async () => {
    process.env.DASHSCOPE_BASE_URL = 'https://z.ai.evil.tld/v1';
    process.env.DASHSCOPE_AUTH_TOKEN = 'test-token';
  ...
  it('returns error when API call fails', async () => {
    process.env.DASHSCOPE_BASE_URL = 'https://api.z.ai/v1';
    process.env.DASHSCOPE_AUTH_TOKEN = 'test-token';
...
describe('isMinimaxHost', () => {
  ...
  it('rejects unrelated hosts', () => {
    expect(isMinimaxHost('https://api.dashscope.aliyuncs.com')).toBe(false);
...
describe('getUsage routing - minimax', () => {
  ...
  it('routes to minimax when DASHSCOPE_BASE_URL is minimax host with MINIMAX_API_KEY', async () => {
    process.env.DASHSCOPE_BASE_URL = 'https://api.minimax.io/anthropic';
  ...
  it('falls back to DASHSCOPE_AUTH_TOKEN when MINIMAX_API_KEY is not set', async () => {
    process.env.DASHSCOPE_BASE_URL = 'https://api.minimax.io/anthropic';
    process.env.DASHSCOPE_AUTH_TOKEN = 'test-auth-token';
  ...
  it('returns no_credentials when minimax host detected but no API key', async () => {
    process.env.DASHSCOPE_BASE_URL = 'https://api.minimax.io/anthropic';
    // Neither MINIMAX_API_KEY nor DASHSCOPE_AUTH_TOKEN set
  ...
  it('does NOT route to minimax for look-alike hosts', async () => {
    process.env.DASHSCOPE_BASE_URL = 'https://minimax.io.evil.tld/v1';
  ...
  it('prefers MINIMAX_API_KEY over DASHSCOPE_AUTH_TOKEN', async () => {
    process.env.DASHSCOPE_BASE_URL = 'https://api.minimax.io/anthropic';
    process.env.MINIMAX_API_KEY = 'preferred-key';
    process.env.DASHSCOPE_AUTH_TOKEN = 'fallback-key';
