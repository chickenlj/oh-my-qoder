...
describe('HUD CLI diagnostic (no stdin, no watch mode)', () => {
  ...
  beforeEach(() => {
    ...
    tempConfigDir = join(tmpdir(), `omq-hud-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  it('shows diagnostic with version and preset when no stdin and no cache', async () => {
    ...
    expect(output).toContain('[OMQ] HUD v4.10.1');
    expect(output).toContain('preset: focused');
    expect(output).not.toContain('run /omq-setup to install properly');
  });

  it('shows HUD script as MISSING when omq-hud.mjs does not exist', async () => {
    const hud = await importHudModule();
    await hud.main(false, false);
  ...
  it('shows HUD script as installed when omq-hud.mjs exists', async () => {
    writeFileSync(join(tempConfigDir, 'hud', 'omq-hud.mjs'), '// stub');
    const hud = await importHudModule();
    await hud.main(false, false);
  ...
  it('shows statusLine as configured when settings.json has omq-hud command', async () => {
    writeFileSync(join(tempConfigDir, 'hud', 'omq-hud.mjs'), '// stub');
    writeFileSync(
      join(tempConfigDir, 'settings.json'),
      JSON.stringify({ statusLine: { type: 'command', command: 'node $HOME/.qoder/hud/omq-hud.mjs' } }),
    );
    const hud = await importHudModule();
    await hud.main(false, false);
    ...
    expect(output).toContain('HUD renders automatically inside Qoder CLI sessions.');
  });
  ...
  it('handles legacy string statusLine format', async () => {
    ...
    writeFileSync(
      join(tempConfigDir, 'settings.json'),
      JSON.stringify({ statusLine: '~/.qoder/hud/omq-hud.mjs' }),
    );
    const hud = await importHudModule();
    await hud.main(false, false);
  ...
  it('shows correct version from getRuntimePackageVersion', async () => {
    ...
    const output = consoleLogSpy.mock.calls.map((c: unknown[]) => c[0]).join('\n');
    expect(output).toContain('[OMQ] HUD v5.0.0');
  });
