...
describe('Notepad Module', () => {
  ...
  describe('initNotepad', () => {
    it('should create notepad.md with correct structure', () => {
      ...
      expect(content).toContain('Auto-managed by OMQ');
    });

    it('should create .omq directory if not exists', () => {
      const omqDir = join(testDir, '.omq');
      expect(existsSync(omqDir)).toBe(false);

      initNotepad(testDir);

      expect(existsSync(omqDir)).toBe(true);
    });
