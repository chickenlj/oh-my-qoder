...
describe('omq config-stop-callback tag options', () => {
  it('updates telegram tagList options and preserves existing config fields', () => {
    const homeDir = mkdtempSync(join(tmpdir(), 'omq-cli-stop-callback-home-'));
    const configPath = join(homeDir, '.qwen', '.omq-config.json');
  ...
  it('uses QODER_CONFIG_DIR for the default file callback path', () => {
    const homeDir = mkdtempSync(join(tmpdir(), 'omq-cli-stop-callback-home-'));
    const claudeConfigDir = join(homeDir, '.qwen-isolated-workspace');
    const configPath = join(claudeConfigDir, '.omq-config.json');
    ...
    const result = spawnSync(process.execPath, ['--import', 'tsx', CLI_ENTRY, 'config-stop-callback', 'file', '--enable'], {
      ...
      env: {
        ...process.env,
        HOME: homeDir,
        QODER_CONFIG_DIR: claudeConfigDir,
      },
      encoding: 'utf-8',
    });
