...
describe('resolveTranscriptPath', () => {
  ...
  beforeEach(() => {
    tempDir = join(tmpdir(), `omq-test-transcript-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  it('resolves worktree paths with complex worktree names', () => {
    ...
    // Worktree with a path-like name (e.g., from OMQ project-session-manager)
    const worktreePath = join(
      tempDir,
      'projects',
      '-home-bellman-Workspace-myproject--claude-worktrees-home-bellman-Workspace-omq-worktrees-fix-issue-1094',
      'session-uuid.jsonl',
    );
