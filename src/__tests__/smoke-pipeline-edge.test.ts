...
describe('EDGE: Pipeline Orchestrator (issue #1132)', () => {
  ...
  beforeEach(() => {
    ...
    // Pipeline state uses getOmqRoot(worktreeRoot) — mock returns <dir>/.omq for any arg
    mockGetOmqRoot.mockImplementation((dir?: string) => {
      const base = dir || testDir;
      const omqDir = join(base, '.omq');
...
describe('EDGE: Shared Memory (issue #1137)', () => {
  ...
  it('listEntries on empty namespace returns empty array', () => {
    ...
    const omqDir = mockGetOmqRoot();
    mkdirSync(join(omqDir, 'state', 'shared-memory', 'empty-ns'), { recursive: true });
  ...
  it('cleanupExpired on empty namespace returns {removed: 0}', () => {
    const omqDir = mockGetOmqRoot();
    mkdirSync(join(omqDir, 'state', 'shared-memory', 'clean-ns'), { recursive: true });
...
describe('EDGE: Config Loader forceInherit (issue #1135)', () => {
  const ORIG = process.env.OMQ_ROUTING_FORCE_INHERIT;

  afterEach(() => {
    if (ORIG === undefined) delete process.env.OMQ_ROUTING_FORCE_INHERIT;
    else process.env.OMQ_ROUTING_FORCE_INHERIT = ORIG;
  });

  it('OMQ_ROUTING_FORCE_INHERIT=TRUE (uppercase) does not enable forceInherit', () => {
  ...
  it('OMQ_ROUTING_FORCE_INHERIT=1 (number string) does not enable forceInherit', () => {
    process.env.OMQ_ROUTING_FORCE_INHERIT = '1';
    const config = loadEnvConfig();
    expect(config.routing?.forceInherit).toBe(false);
  });

  it('OMQ_ROUTING_FORCE_INHERIT=yes is not truthy', () => {
    process.env.OMQ_ROUTING_FORCE_INHERIT = 'yes';
    const config = loadEnvConfig();
    expect(config.routing?.forceInherit).toBe(false);
  });

  it('OMQ_ROUTING_FORCE_INHERIT=" true " (whitespace) does not enable forceInherit', () => {
    process.env.OMQ_ROUTING_FORCE_INHERIT = ' true ';
    const config = loadEnvConfig();
    expect(config.routing?.forceInherit).toBe(false);
  });

  it('OMQ_ROUTING_FORCE_INHERIT="" (empty string) sets forceInherit to false', () => {
    process.env.OMQ_ROUTING_FORCE_INHERIT = '';
    const config = loadEnvConfig();
  ...
  it('multiple env vars set simultaneously: all are reflected', () => {
    process.env.OMQ_ROUTING_FORCE_INHERIT = 'true';
    process.env.OMQ_ROUTING_ENABLED = 'false';
    process.env.OMQ_ROUTING_DEFAULT_TIER = 'HIGH';
    ...
    // Clean up extra vars
    delete process.env.OMQ_ROUTING_ENABLED;
    delete process.env.OMQ_ROUTING_DEFAULT_TIER;
  });
});
