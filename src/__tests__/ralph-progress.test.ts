...
describe('Ralph Progress Module', () => {
  ...
  describe('initProgress', () => {
    it('should create progress.txt in .omq directory', () => {
      expect(initProgress(testDir)).toBe(true);
      expect(existsSync(join(testDir, '.omq', PROGRESS_FILENAME))).toBe(true);
    });
  ...
  describe('readProgressRaw / readProgress', () => {
    ...
    it('should read progress from .omq directory', () => {
      const omqDir = join(testDir, '.omq');
      mkdirSync(omqDir, { recursive: true });
      writeFileSync(join(omqDir, PROGRESS_FILENAME), '# Test');
      expect(readProgressRaw(testDir)).toBe('# Test');
    });
  });
