#!/bin/sh
# OMQ Node.js Finder (find-node.sh)
#
...
# shells (e.g. Qoder CLI hook invocations). Fixes issue #892.
#
# Priority:
#   1. nodeBinary stored in ~/.qoder/.omq-config.json (set at setup time)
...
#
# Exits 0 on failure so it never blocks Qoder CLI hook processing.
...
# ---------------------------------------------------------------------------
# 1. Read stored node path from OMQ config
# ---------------------------------------------------------------------------
CLAUDE_DIR="$(resolve_claude_config_dir)"
CONFIG_FILE="$CLAUDE_DIR/.omq-config.json"
...
# ---------------------------------------------------------------------------
if [ -z "$NODE_BIN" ]; then
  printf '[OMQ] Error: Could not find node binary. Run /oh-my-qoder:omq-setup to fix.\n' >&2
  exit 0  # exit 0 so this hook does not block Qoder CLI
fi
