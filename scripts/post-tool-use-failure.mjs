#!/usr/bin/env node
// OMQ Post-Tool-Use-Failure Hook (Node.js)
...
/**
 * Resolve state file paths for a given omq root and optional session id.
 * Session-scoped: <omqRoot>/state/sessions/<sid>/last-tool-error-state.json
 * Legacy:         <omqRoot>/state/last-tool-error.json
 *
 * @param {string} omqRoot
 * @param {string|undefined} sessionId
 * @returns {{ statePath: string, stateDir: string }}
 */
function resolveErrorStatePaths(omqRoot, sessionId) {
  if (sessionId) {
...
// Initialize .omq directory if needed; returns the omq root (not state subdir)
async function initOmcDir(directory) {
  ...
  const omqDir = await resolveOmqStateRoot(directory);
  const stateDir = join(omqDir, 'state');

  if (!existsSync(omqDir)) {
    try { mkdirSync(omqDir, { recursive: true }); } catch {}
  }
...
async function main() {
  ...
  try {
    ...
    const omqRoot = await initOmcDir(directory);

    // Resolve state paths (session-scoped or legacy)
    const { stateDir, statePath } = resolveErrorStatePaths(omqRoot, sessionId);
