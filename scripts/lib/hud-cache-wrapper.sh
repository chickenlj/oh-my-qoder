#!/bin/sh
# OMQ HUD cached statusLine launcher.
#
# Qoder CLI invokes statusLine commands for every render. Starting Node and
...
CACHE_DIR=${OMQ_HUD_CACHE_DIR:-"$CONFIG_DIR/hud/cache"}
HUD_SCRIPT=${1:-"$SCRIPT_DIR/omq-hud.mjs"}
INPUT_TMP="$CACHE_DIR/stdin.$$.tmp"
LOCK_STALE_SECONDS=${OMQ_HUD_LOCK_STALE_SECONDS:-10}
...
if [ -s "$OUTPUT_FILE" ]; then
  cat "$OUTPUT_FILE" 2>/dev/null || printf '[OMQ] Starting...\n'
  # Refresh in background for the next frame.
  if try_acquire_lock; then
    if [ "${OMQ_HUD_SYNC_REFRESH:-0}" = "1" ]; then
      refresh_cache
    else
...
# sees the real HUD from the first frame. Qoder CLI v2.1.x does not
# re-poll the statusLine until user interaction, so an async background
# refresh leaves the pane stuck on "[OMQ] Starting..." until they type.
