...
export function getPreflightContextThreshold(env = process.env) {
  const parsed = Number.parseInt(env.OMQ_AGENT_PREFLIGHT_CONTEXT_THRESHOLD || '72', 10);
...
export function buildPreflightRecoveryAdvice(contextPercent, threshold = DEFAULT_PREFLIGHT_CONTEXT_THRESHOLD) {
  return `[OMQ] Preflight context guard: ${contextPercent}% used ` +
    ...
    `(3) if compact fails, open a fresh session and continue from .omq/state + .omq/notepad.md.`;
}
