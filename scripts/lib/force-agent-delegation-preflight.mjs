...
//
// Opt-in: default OFF. Enable via `.omq/config.json`:
//
//   {
//     "routing": {
//       "forceDelegation": {
...
//             "denyMessage": "10+ raw Reads in 2 min — spawn Agent(subagent_type='oh-my-qoder:explore', model='low', ...). Bypass: ALLOW_RAW_READ=1.",
 ...
 *
 * @param {object} args
 * @param {string} args.toolName - Qoder CLI tool name (Read|Edit|Write|...)
 * @param {string} [args.stateDir] - Directory used to persist the event window.
 *   Typically `<cwd>/.omq/state` to mirror the rest of OMQ state storage.
 * @param {object} [args.env=process.env] - Environment for bypass-flag lookup.
 * @param {Function} [args.loadOmcConfig] - Function returning the resolved OMQ
        ...
        return {
          ...
          reason:
            typeof rule.denyMessage === 'string' && rule.denyMessage
              ? rule.denyMessage
              : `[OMQ] Force-agent-delegation: ${observed} ${toolName} in last ${windowSeconds}s ` +
