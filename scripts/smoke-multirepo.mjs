...
// Fixture setup
// --------------------------------------------------------------------------
const FIXTURE = join(tmpdir(), `omq-multirepo-smoke-${process.pid}`);
...
writeFileSync(join(FIXTURE, '.omq-workspace'), JSON.stringify({ id: 'smoke-test' }));
...
// --------------------------------------------------------------------------
// Step 1 — findWorkspaceRoot / getOmqRoot / getProjectIdentifier from api/
// --------------------------------------------------------------------------
console.log('\n=== STEP 1: API dir — workspace root + omq root + project identifier ===');
...
assert(
  ...
  'findWorkspaceRoot must walk up from sub-git-repo to .omq-workspace marker'
);

clearWorktreeCache();
const omqRoot = getOmqRoot(apiDir);
const expectedOmcRoot = join(FIXTURE, '.omq');
assert(
  resolve(omqRoot) === resolve(expectedOmcRoot),
  'getOmqRoot(apiDir) → fixture/.omq',
  'getOmqRoot(apiDir) → WRONG',
  `returned: ${omqRoot}  expected: ${expectedOmcRoot}`,
  'getOmqRoot must anchor to workspace marker dir, not api/ git root'
);
...
const expectedApiWrite = join(FIXTURE, '.omq', 'state', 'sessions', 'session-api', 'demo-state.json');
assert(
  ...
  'effectiveWrite must be session-scoped under shared .omq/'
);

// --------------------------------------------------------------------------
// Step 2 — web/ paths same .omq, different session subdir
// --------------------------------------------------------------------------
console.log('\n=== STEP 2: WEB dir — same .omq, different session ===');

clearWorktreeCache();
const webPaths = resolveSessionStatePaths('demo', 'session-web', webDir);
const expectedWebWrite = join(FIXTURE, '.omq', 'state', 'sessions', 'session-web', 'demo-state.json');
...
assert(
  ...
  'effectiveWrite must be under shared .omq/ with distinct session subdir'
);
...
// ultragoal/artifacts.ts uses getOmqRoot(cwd) — plans land in the shared
// workspace-marker root when .omq-workspace is present (fixed in multi-repo
// rollout). The CLI sets cwd = subprocess cwd; resolution flows through
// getOmqRoot which honors OMQ_STATE_DIR > .omq-workspace > git > cwd.
...
function runUltragoal(cwd, sessionId, brief) {
  return spawnSync(
    ...
    {
      cwd,
      env: { ...process.env, OMQ_SESSION_ID: sessionId },
      encoding: 'utf-8',
      timeout: 30000,
    }
  );
...
// Actual write locations: <subrepo>/.omq/ultragoal/plans/<planId>/
const apiPlansDir = join(apiDir, '.omq', 'ultragoal', 'plans');
const webPlansDir = join(webDir, '.omq', 'ultragoal', 'plans');
const sharedPlansDir = join(FIXTURE, '.omq', 'ultragoal', 'plans');
...
// After multi-repo Wave A: ultragoal plans now land in the shared workspace .omq/
// because artifacts.ts was updated to use getOmqRoot()/workspace-marker resolution.
// Plans from BOTH subrepos go to the shared FIXTURE/.omq/ultragoal/plans/.
const totalPlans = apiPlanDirs.length + webPlanDirs.length + sharedPlanDirs.length;
if (sharedPlanDirs.length >= 2) {
  pass(
    `ultragoal plans land in shared workspace .omq/ (${sharedPlanDirs.length} plans)`,
...
const fakeStateDir = join(FIXTURE, '.omq', 'state', 'sessions', fakeSid);
...
// --------------------------------------------------------------------------
console.log('\n=== STEP 5: Session subdir contents (shared .omq/) ===');
const sessionsDir = join(FIXTURE, '.omq', 'state', 'sessions');
if (existsSync(sessionsDir)) {
  ...
  console.log('  Sessions in shared fixture .omq/:');
...
// --------------------------------------------------------------------------
console.log('\n=== STEP 6: Windows backslash path with OMQ_STATE_DIR ===');

if (process.platform === 'win32') {
  const winStateDir = join(FIXTURE, 'win-state');
  mkdirSync(winStateDir, { recursive: true });
  const origStateDirEnv = process.env.OMQ_STATE_DIR;
  process.env.OMQ_STATE_DIR = winStateDir;
  clearWorktreeCache();

  const winPaths = resolveSessionStatePaths('test', 'win-session', apiDir);
  const writePath = winPaths.effectiveWrite;

  // Assert: no resolved write path contains \.omq\ OUTSIDE the configured OMQ_STATE_DIR root
  const containsOmcOutsideRoot =
    writePath.includes('\\.omq\\') &&
    !writePath.startsWith(winStateDir);

  assert(
    !containsOmcOutsideRoot,
    'Windows: resolved write path does not contain \\.omq\\ outside configured OMQ_STATE_DIR',
    'Windows: resolved write path contains \\.omq\\ OUTSIDE OMQ_STATE_DIR (path escape bug)',
    `writePath=${writePath}  stateDir=${winStateDir}`,
    'When OMQ_STATE_DIR is set, no path should escape to a raw /.omq/ location'
  );
  console.log(`        write path: ${writePath}`);

  // Restore
  if (origStateDirEnv === undefined) delete process.env.OMQ_STATE_DIR;
  else process.env.OMQ_STATE_DIR = origStateDirEnv;
  clearWorktreeCache();
} else {
...
// --------------------------------------------------------------------------
// Step 7 — Workspace-marker retrofit: pre-existing sibling .omq/state/
// --------------------------------------------------------------------------
console.log('\n=== STEP 7: Workspace-marker retrofit sibling-scan warning ===');

const retrofitFixture = join(tmpdir(), `omq-retrofit-smoke-${process.pid}`);
...
// Pre-create sibling .omq/state/ content BEFORE dropping workspace marker
const legacyStateDir = join(retrofitApi, '.omq', 'state');
...
writeFileSync(join(retrofitFixture, '.omq-workspace'), JSON.stringify({ id: 'retrofit-test' }));
...
// Trigger via warnSiblingRetrofit directly (lifted off getOmqRoot hot path)
...
// (i) Structured warning emitted listing sibling .omq/state dirs
assert(
  retrofitStderr.includes('[omq] workspace-retrofit warning'),
  'Retrofit warning emitted to stderr',
  'Retrofit warning NOT emitted to stderr',
  `stderr captured: ${retrofitStderr.slice(0, 200)}`,
  'warnSiblingRetrofit must fire when sibling has pre-existing .omq/state/'
);

// (ii) Pre-existing api/.omq/state/ralph-state.json not overwritten or deleted
assert(
...
assert(
  retrofitStderr.includes('OMQ_MIGRATE_LEGACY_STATE=1'),
  'Retrofit warning includes OMQ_MIGRATE_LEGACY_STATE=1 migration command',
  'Retrofit warning missing OMQ_MIGRATE_LEGACY_STATE=1 migration hint',
...
// --------------------------------------------------------------------------
// Step 8 — Template drift simulation: AST-grep gate red on raw .omq pattern
// --------------------------------------------------------------------------
console.log('\n=== STEP 8: AST-grep gate — drift fixture triggers non-zero exit ===');

const driftFixtureDir = join(tmpdir(), `omq-drift-gate-${process.pid}`);
mkdirSync(driftFixtureDir, { recursive: true });
const driftFixtureFile = join(driftFixtureDir, 'drift-fixture.mjs');
writeFileSync(
  driftFixtureFile,
  `import {join} from 'path'; const dir = '/tmp'; const p = join(dir, '.omq', 'state', 'foo');\n`
);
...
assert(
  ...
  'AST-grep gate exits ZERO on drift fixture (failed to detect raw .omq pattern)',
  `exit=${gateResult.status}\nstdout=${gateResult.stdout?.slice(0, 300)}\nstderr=${gateResult.stderr?.slice(0, 300)}`,
  'Gate must exit non-zero when raw join(...,.omq,...) is found outside whitelist'
);
...
assert(
  gateResult.stderr?.includes("join(dir, '.omq'") || gateResult.stderr?.includes('.omq'),
...
if (failed === 0) {
  ...
  console.log('  state) anchor to .omq-workspace marker when present.\n');
} else {
