...
function findOrphanProcesses(filterTeam) {
  ...
  try {
    if (process.platform === 'win32') {
      ...
      for (const line of output.split('\n')) {
        // Match OMQ agent processes with team context (exclude bare 'node' to avoid over-matching)
        if ((line.includes('qwen') || line.includes('codex') || line.includes('gemini') || line.includes('omq') || line.includes('oh-my-claude'))) {
 ...
 * hang waiting for the escalation window to elapse. In practice, OMQ agent
 * processes (claude/codex/gemini/omq) respect SIGTERM and exit within
